...
#' @param AllowClassicFlow Enables or disables the Basic (Classic) authentication flow. For more information, see [Identity Pools (Federated Identities) Authentication Flow](https://docs.aws.amazon.com/cognito/latest/developerguide/authentication-flow.html) in the *Amazon Cognito Developer Guide*.
...
#' 
#' You can use [`get_open_id_token_for_developer_identity`][cognitoidentity_get_open_id_token_for_developer_identity] to create a new identity and to link new logins (that is, user credentials issued by a public provider or developer provider) to an existing identity. When you want to create a new identity, the `IdentityId` should be null. When you want to associate a new login with an existing authenticated/unauthenticated identity, you can do so by providing the existing `IdentityId`. This API will create the identity in the specified `IdentityPoolId`.
#' 
#' You must use Amazon Web Services developer credentials to call this operation.
#'
#' @usage
...
#'
#' @param IdentityPoolId &#91;required&#93; You can use this operation to get the ID of the Identity Pool you setup attribute mappings for.
...
#'
#' @param ResourceArn &#91;required&#93; The Amazon Resource Name (ARN) of the identity pool that the tags are assigned to.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' [`lookup_developer_identity`][cognitoidentity_lookup_developer_identity] is intended for low-throughput control plane operations: for example, to enable customer service to locate an identity ID by username. If you are using it for higher-volume operations such as user authentication, your requests are likely to be throttled. [`get_open_id_token_for_developer_identity`][cognitoidentity_get_open_id_token_for_developer_identity] is a better option for higher-volume operations for user authentication.
#' 
#' You must use Amazon Web Services developer credentials to call this operation.
#'
#' @usage
...
#' @param UseDefaults You can use this operation to use default (username and clientID) attribute mappings.
