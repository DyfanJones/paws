...
#' 
#' Before you can call this operation, you must first enable Health to work with Organizations. To do this, call the [`enable_health_service_access_for_organization`][health_enable_health_service_access_for_organization] operation from your organization's management account.
#' 
...
#' Before you can call this operation, you must first enable Health to work with Organizations. To do this, call the [`enable_health_service_access_for_organization`][health_enable_health_service_access_for_organization] operation from your organization's management account.
#' 
#' When you call the [`describe_event_details_for_organization`][health_describe_event_details_for_organization] operation, specify the `organizationEventDetailFilters` object in the request. Depending on the Health event type, note the following differences:
#' 
...
#' 
#' -   When you call the [`describe_events`][health_describe_events] operation and specify an entity for the `entityValues` parameter, Health might return public events that aren't specific to that resource. For example, if you call [`describe_events`][health_describe_events] and specify an ID for an Amazon Elastic Compute Cloud (Amazon EC2) instance, Health might return events that aren't specific to that resource or service. To get events that are specific to a service, use the `services` parameter in the `filter` object. For more information, see [Event](https://docs.aws.amazon.com/health/latest/APIReference/API_Event.html).
#' 
#' -   This API operation uses pagination. Specify the `nextToken` parameter in the next request to return more results.
#'
#' @usage
