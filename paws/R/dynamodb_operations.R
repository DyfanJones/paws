...
#' 
#' A single operation can retrieve up to 16 MB of data, which can contain as many as 100 items. [`batch_get_item`][dynamodb_batch_get_item] returns a partial result if the response size limit is exceeded, the table's provisioned throughput is exceeded, more than 1MB per partition is requested, or an internal processing failure occurs. If a partial result is returned, the operation returns a value for `UnprocessedKeys`. You can use this value to retry the operation starting with the next item to get.
#' 
#' If you request more than 100 items, [`batch_get_item`][dynamodb_batch_get_item] returns a `ValidationException` with the message "Too many items requested for the BatchGetItem call."
#' 
#' For example, if you ask to retrieve 100 items, but each individual item is 300 KB in size, the system returns 52 items (so as not to exceed the 16 MB limit). It also returns an appropriate `UnprocessedKeys` value so you can get the next page of results. If desired, your application can include its own logic to assemble the pages of results into one dataset.
#' 
#' If *none* of the items can be processed due to insufficient provisioned throughput on all of the tables in the request, then [`batch_get_item`][dynamodb_batch_get_item] returns a `ProvisionedThroughputExceededException`. If *at least one* of the items is successfully processed, then [`batch_get_item`][dynamodb_batch_get_item] completes successfully, while returning the keys of the unread items in `UnprocessedKeys`.
#' 
#' If DynamoDB returns any unprocessed items, you should retry the batch operation on those items. However, *we strongly recommend that you use an exponential backoff algorithm*. If you retry the batch operation immediately, the underlying read or write requests can still fail due to throttling on the individual tables. If you delay the batch operation using exponential backoff, the individual requests in the batch are much more likely to succeed.
#' 
#' For more information, see [Batch Operations and Error Handling](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Programming.Errors.html#BatchOperations) in the *Amazon DynamoDB Developer Guide*.
#' 
#' By default, [`batch_get_item`][dynamodb_batch_get_item] performs eventually consistent reads on every table in the request. If you want strongly consistent reads instead, you can set `ConsistentRead` to `true` for any or all tables.
#' 
#' In order to minimize response latency, [`batch_get_item`][dynamodb_batch_get_item] may retrieve items in parallel.
#' 
...
#' 
#' [`batch_get_item`][dynamodb_batch_get_item] will result in a `ValidationException` if the same key is specified multiple times.
#'
#' @usage
...
#' 
#' [`batch_write_item`][dynamodb_batch_write_item] cannot update items. If you perform a [`batch_write_item`][dynamodb_batch_write_item] operation on an existing item, that item's values will be overwritten by the operation and it will appear like it was updated. To update items, we recommend you use the [`update_item`][dynamodb_update_item] action.
#' 
...
#' If DynamoDB returns any unprocessed items, you should retry the batch operation on those items. However, *we strongly recommend that you use an exponential backoff algorithm*. If you retry the batch operation immediately, the underlying read or write requests can still fail due to throttling on the individual tables. If you delay the batch operation using exponential backoff, the individual requests in the batch are much more likely to succeed.
#' 
#' For more information, see [Batch Operations and Error Handling](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Programming.Errors.html#Programming.Errors.BatchOperations) in the *Amazon DynamoDB Developer Guide*.
#' 
#' With [`batch_write_item`][dynamodb_batch_write_item], you can efficiently write or delete large amounts of data, such as from Amazon EMR, or copy data from another database into DynamoDB. In order to improve performance with these large-scale operations, [`batch_write_item`][dynamodb_batch_write_item] does not behave in the same way as individual [`put_item`][dynamodb_put_item] and [`delete_item`][dynamodb_delete_item] calls would. For example, you cannot specify conditions on individual put and delete requests, and [`batch_write_item`][dynamodb_batch_write_item] does not return deleted items in the response.
#' 
#' If you use a programming language that supports concurrency, you can use threads to write items in parallel. Your application must include the necessary logic to manage the threads. With languages that don't support threading, you must update or delete the specified items one at a time. In both situations, [`batch_write_item`][dynamodb_batch_write_item] performs the specified put and delete operations in parallel, giving you the power of the thread pool approach without having to introduce complexity into your application.
#' 
#' Parallel processing reduces latency, but each specified put and delete request consumes the same number of write capacity units whether it is processed in parallel or not. Delete operations on nonexistent items consume one write capacity unit.
#' 
#' If one or more of the following is true, DynamoDB rejects the entire batch write operation:
#' 
#' -   One or more tables specified in the [`batch_write_item`][dynamodb_batch_write_item] request does not exist.
#' 
#' -   Primary key attributes specified on an item in the request do not match those in the corresponding table's primary key schema.
#' 
#' -   You try to perform multiple operations on the same item in the same [`batch_write_item`][dynamodb_batch_write_item] request. For example, you cannot put and delete the same item in the same [`batch_write_item`][dynamodb_batch_write_item] request.
#' 
...
#' 
#' You can call [`create_backup`][dynamodb_create_backup] at a maximum rate of 50 times per second.
#' 
...
#' 
#' To determine which version you're using, see [Determining the global table version you are using](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/V2globaltables_versions.html). To update existing global tables from version 2017.11.29 (Legacy) to version 2019.11.21 (Current), see [Upgrading global tables](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/V2globaltables_versions.html).
#' 
...
#' 
#' [`create_table`][dynamodb_create_table] is an asynchronous operation. Upon receiving a [`create_table`][dynamodb_create_table] request, DynamoDB immediately returns a response with a `TableStatus` of `CREATING`. After the table is created, DynamoDB sets the `TableStatus` to `ACTIVE`. You can perform read and write operations only on an `ACTIVE` table.
#' 
#' You can optionally define secondary indexes on the new table, as part of the [`create_table`][dynamodb_create_table] operation. If you want to create multiple tables with secondary indexes on them, you must create the tables sequentially. Only one table with secondary indexes can be in the `CREATING` state at any given time.
#' 
#' You can use the [`describe_table`][dynamodb_describe_table] action to check the table status.
#'
#' @usage
...
#' Deletes an existing backup of a table.
#' 
#' You can call [`delete_backup`][dynamodb_delete_backup] at a maximum rate of 10 times per second.
#'
#' @usage
...
#' 
#' Unless you specify conditions, the [`delete_item`][dynamodb_delete_item] is an idempotent operation; running it multiple times on the same item or attribute does *not* result in an error response.
#' 
...
#' 
#' [`delete_resource_policy`][dynamodb_delete_resource_policy] is an idempotent operation; running it multiple times on the same resource *doesn't* result in an error response, unless you specify an `ExpectedRevisionId`, which will then return a `PolicyNotFoundException`.
#' 
#' To make sure that you don't inadvertently lock yourself out of your own resources, the root principal in your Amazon Web Services account can perform [`delete_resource_policy`][dynamodb_delete_resource_policy] requests, even if your resource-based policy explicitly denies the root principal's access.
#' 
#' [`delete_resource_policy`][dynamodb_delete_resource_policy] is an asynchronous operation. If you issue a [`get_resource_policy`][dynamodb_get_resource_policy] request immediately after running the [`delete_resource_policy`][dynamodb_delete_resource_policy] request, DynamoDB might still return the deleted policy. This is because the policy for your resource might not have been deleted yet. Wait for a few seconds, and then try the [`get_resource_policy`][dynamodb_get_resource_policy] request again.
#'
#' @usage
...
#' 
#' Use the [`describe_table`][dynamodb_describe_table] action to check the status of the table.
#'
#' @usage
...
#' Describes an existing backup of a table.
#' 
#' You can call [`describe_backup`][dynamodb_describe_backup] at a maximum rate of 10 times per second.
#'
#' @usage
...
#' 
#' You can call [`describe_continuous_backups`][dynamodb_describe_continuous_backups] at a maximum rate of 10 times per second.
#'
#' @usage
...
#' 
#' 1.  Call [`describe_limits`][dynamodb_describe_limits] for a particular Region to obtain your current account quotas on provisioned capacity there.
#' 
...
#' 
#' 3.  Call [`list_tables`][dynamodb_list_tables] to obtain a list of all your DynamoDB tables.
#' 
#' 4.  For each table name listed by [`list_tables`][dynamodb_list_tables], do the following:
#' 
#'     -   Call [`describe_table`][dynamodb_describe_table] with the table name.
#' 
#'     -   Use the data returned by [`describe_table`][dynamodb_describe_table] to add the read capacity units and write capacity units provisioned for the table itself to your variables.
#' 
#'     -   If the table has one or more global secondary indexes (GSIs), loop over these GSIs and add their provisioned capacity values to your variables as well.
#' 
#' 5.  Report the account quotas for that Region returned by [`describe_limits`][dynamodb_describe_limits], along with the total current provisioned capacity levels you have calculated.
#' 
...
#' 
#' [`describe_limits`][dynamodb_describe_limits] should only be called periodically. You can expect throttling errors if you call it more than once in a minute.
#' 
#' The [`describe_limits`][dynamodb_describe_limits] Request element has no content.
#'
#' @usage
...
#' 
#' If you issue a [`describe_table`][dynamodb_describe_table] request immediately after a [`create_table`][dynamodb_create_table] request, DynamoDB might return a `ResourceNotFoundException`. This is because [`describe_table`][dynamodb_describe_table] uses an eventually consistent query, and the metadata for your table might not be available at that moment. Wait for a few seconds, and then try the [`describe_table`][dynamodb_describe_table] request again.
#'
#' @usage
...
#' 
#' The entire transaction must consist of either read statements or write statements, you cannot mix both in one transaction. The EXISTS function is an exception and can be used to check the condition of specific attributes of the item in a similar manner to `ConditionCheck` in the [`transact_write_items`][dynamodb_transact_write_items] API.
#'
#' @usage
...
#' 
#' [`get_item`][dynamodb_get_item] provides an eventually consistent read by default. If your application requires a strongly consistent read, set `ConsistentRead` to `true`. Although a strongly consistent read might take more time than an eventually consistent read, it always returns the last updated value.
#'
#' @usage
...
#' 
#' [`get_resource_policy`][dynamodb_get_resource_policy] follows an [*eventually consistent*](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html) model. The following list describes the outcomes when you issue the [`get_resource_policy`][dynamodb_get_resource_policy] request immediately after issuing another request:
#' 
#' -   If you issue a [`get_resource_policy`][dynamodb_get_resource_policy] request immediately after a [`put_resource_policy`][dynamodb_put_resource_policy] request, DynamoDB might return a `PolicyNotFoundException`.
#' 
#' -   If you issue a [`get_resource_policy`][dynamodb_get_resource_policy]request immediately after a [`delete_resource_policy`][dynamodb_delete_resource_policy] request, DynamoDB might return the policy that was present before the deletion request.
#' 
#' -   If you issue a [`get_resource_policy`][dynamodb_get_resource_policy] request immediately after a [`create_table`][dynamodb_create_table] request, which includes a resource-based policy, DynamoDB might return a `ResourceNotFoundException` or a `PolicyNotFoundException`.
#' 
#' Because [`get_resource_policy`][dynamodb_get_resource_policy] uses an *eventually consistent* query, the metadata for your policy or table might not be available at that moment. Wait for a few seconds, and then retry the [`get_resource_policy`][dynamodb_get_resource_policy] request.
#' 
#' After a [`get_resource_policy`][dynamodb_get_resource_policy] request returns a policy created using the [`put_resource_policy`][dynamodb_put_resource_policy] request, the policy will be applied in the authorization of requests to the resource. Because this process is eventually consistent, it will take some time to apply the policy to all requests to a resource. Policies that you attach while creating a table using the [`create_table`][dynamodb_create_table] request will always be applied to all requests for that table.
#'
#' @usage
...
#' 
#' To determine whether [`put_item`][dynamodb_put_item] overwrote an existing item, use `ReturnValues` set to `ALL_OLD`. If the response includes the `Attributes` element, an existing item was overwritten.
#' 
#' For more information about [`put_item`][dynamodb_put_item], see [Working with Items](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithItems.html) in the *Amazon DynamoDB Developer Guide*.
#'
#' @usage
...
#' 
#' [`put_resource_policy`][dynamodb_put_resource_policy] is an idempotent operation; running it multiple times on the same resource using the same policy document will return the same revision ID. If you specify an `ExpectedRevisionId` that doesn't match the current policy's `RevisionId`, the `PolicyNotFoundException` will be returned.
#' 
#' [`put_resource_policy`][dynamodb_put_resource_policy] is an asynchronous operation. If you issue a [`get_resource_policy`][dynamodb_get_resource_policy] request immediately after a [`put_resource_policy`][dynamodb_put_resource_policy] request, DynamoDB might return your previous policy, if there was one, or return the `PolicyNotFoundException`. This is because [`get_resource_policy`][dynamodb_get_resource_policy] uses an eventually consistent query, and the metadata for your policy or table might not be available at that moment. Wait for a few seconds, and then try the [`get_resource_policy`][dynamodb_get_resource_policy] request again.
#'
#' @usage
...
#' 
#' Use the `KeyConditionExpression` parameter to provide a specific value for the partition key. The [`query`][dynamodb_query] operation will return all of the items from the table or index with that partition key value. You can optionally narrow the scope of the [`query`][dynamodb_query] operation by specifying a sort key value and a comparison operator in `KeyConditionExpression`. To further refine the [`query`][dynamodb_query] results, you can optionally provide a `FilterExpression`. A `FilterExpression` determines which items within the results should be returned to you. All of the other results are discarded.
#' 
#' A [`query`][dynamodb_query] operation always returns a result set. If no matching items are found, the result set will be empty. Queries that do not return results consume the minimum number of read capacity units for that type of read operation.
#' 
...
#' 
#' A single [`query`][dynamodb_query] operation will read up to the maximum number of items set (if using the `Limit` parameter) or a maximum of 1 MB of data and then apply any filtering to the results using `FilterExpression`. If `LastEvaluatedKey` is present in the response, you will need to paginate the result set. For more information, see [Paginating the Results](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Query.html#Query.Pagination) in the *Amazon DynamoDB Developer Guide*.
#' 
#' `FilterExpression` is applied after a [`query`][dynamodb_query] finishes, but before the results are returned. A `FilterExpression` cannot contain partition key or sort key attributes. You need to specify those attributes in the `KeyConditionExpression`.
#' 
#' A [`query`][dynamodb_query] operation can return an empty result set and a `LastEvaluatedKey` if all the items read for the page of results are filtered out.
#' 
...
#' 
#' You can call [`restore_table_from_backup`][dynamodb_restore_table_from_backup] at a maximum rate of 10 times per second.
#' 
...
#' 
#' A single [`scan`][dynamodb_scan] operation first reads up to the maximum number of items set (if using the `Limit` parameter) or a maximum of 1 MB of data and then applies any filtering to the results if a `FilterExpression` is provided. If `LastEvaluatedKey` is present in the response, pagination is required to complete the full table scan. For more information, see [Paginating the Results](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.Pagination) in the *Amazon DynamoDB Developer Guide*.
#' 
#' [`scan`][dynamodb_scan] operations proceed sequentially; however, for faster performance on a large table or secondary index, applications can request a parallel [`scan`][dynamodb_scan] operation by providing the `Segment` and `TotalSegments` parameters. For more information, see [Parallel Scan](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.ParallelScan) in the *Amazon DynamoDB Developer Guide*.
#' 
#' By default, a [`scan`][dynamodb_scan] uses eventually consistent reads when accessing the items in a table. Therefore, the results from an eventually consistent [`scan`][dynamodb_scan] may not include the latest item changes at the time the scan iterates through each item in the table. If you require a strongly consistent read of each item as the scan iterates through the items in the table, you can set the `ConsistentRead` parameter to true. Strong consistency only relates to the consistency of the read at the item level.
#' 
...
#' 
#' -   [`tag_resource`][dynamodb_tag_resource] is an asynchronous operation. If you issue a [`list_tags_of_resource`][dynamodb_list_tags_of_resource] request immediately after a [`tag_resource`][dynamodb_tag_resource] request, DynamoDB might return your previous tag set, if there was one, or an empty tag set. This is because [`list_tags_of_resource`][dynamodb_list_tags_of_resource] uses an eventually consistent query, and the metadata for your tags or table might not be available at that moment. Wait for a few seconds, and then try the [`list_tags_of_resource`][dynamodb_list_tags_of_resource] request again.
#' 
#' -   The application or removal of tags using [`tag_resource`][dynamodb_tag_resource] and [`untag_resource`][dynamodb_untag_resource] APIs is eventually consistent. [`list_tags_of_resource`][dynamodb_list_tags_of_resource] API will only reflect the changes after a few seconds.
#' 
#' For an overview on tagging DynamoDB resources, see [Tagging for DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Tagging.html) in the *Amazon DynamoDB Developer Guide*.
#'
#' @usage
...
#' 
#' DynamoDB rejects the entire [`transact_get_items`][dynamodb_transact_get_items] request if any of the following is true:
#' 
...
#' 
#' -   `Put`  —   Initiates a [`put_item`][dynamodb_put_item] operation to write a new item. This structure specifies the primary key of the item to be written, the name of the table to write it in, an optional condition expression that must be satisfied for the write to succeed, a list of the item's attributes, and a field indicating whether to retrieve the item's attributes if the condition is not met.
#' 
#' -   `Update`  —   Initiates an [`update_item`][dynamodb_update_item] operation to update an existing item. This structure specifies the primary key of the item to be updated, the name of the table where it resides, an optional condition expression that must be satisfied for the update to succeed, an expression that defines one or more attributes to be updated, and a field indicating whether to retrieve the item's attributes if the condition is not met.
#' 
#' -   `Delete`  —   Initiates a [`delete_item`][dynamodb_delete_item] operation to delete an existing item. This structure specifies the primary key of the item to be deleted, the name of the table where it resides, an optional condition expression that must be satisfied for the deletion to succeed, and a field indicating whether to retrieve the item's attributes if the condition is not met.
#' 
#' -   `ConditionCheck`  —   Applies a condition to an item that is not being modified by the transaction. This structure specifies the primary key of the item to be checked, the name of the table where it resides, a condition expression that must be satisfied for the transaction to succeed, and a field indicating whether to retrieve the item's attributes if the condition is not met.
#' 
#' DynamoDB rejects the entire [`transact_write_items`][dynamodb_transact_write_items] request if any of the following is true:
#' 
...
#' 
#' -   [`untag_resource`][dynamodb_untag_resource] is an asynchronous operation. If you issue a [`list_tags_of_resource`][dynamodb_list_tags_of_resource] request immediately after an [`untag_resource`][dynamodb_untag_resource] request, DynamoDB might return your previous tag set, if there was one, or an empty tag set. This is because [`list_tags_of_resource`][dynamodb_list_tags_of_resource] uses an eventually consistent query, and the metadata for your tags or table might not be available at that moment. Wait for a few seconds, and then try the [`list_tags_of_resource`][dynamodb_list_tags_of_resource] request again.
#' 
#' -   The application or removal of tags using [`tag_resource`][dynamodb_tag_resource] and [`untag_resource`][dynamodb_untag_resource] APIs is eventually consistent. [`list_tags_of_resource`][dynamodb_list_tags_of_resource] API will only reflect the changes after a few seconds.
#' 
#' For an overview on tagging DynamoDB resources, see [Tagging for DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Tagging.html) in the *Amazon DynamoDB Developer Guide*.
...
#' To determine which version you're using, see [Determining the global table version you are using](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/V2globaltables_versions.html). To update existing global tables from version 2017.11.29 (Legacy) to version 2019.11.21 (Current), see [Upgrading global tables](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/V2globaltables_versions.html).
#' 
#' If you are using global tables [Version 2019.11.21](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/GlobalTables.html) (Current) you can use [`update_table`][dynamodb_update_table] instead.
#' 
#' Although you can use [`update_global_table`][dynamodb_update_global_table] to add replicas and remove replicas in a single request, for simplicity we recommend that you issue separate requests for adding or removing replicas.
#' 
#' If global secondary indexes are specified, then the following conditions must also be met:
#' 
#' -   The global secondary indexes must have the same name.
...
#' 
#' You can also return the item's attribute values in the same [`update_item`][dynamodb_update_item] operation using the `ReturnValues` parameter.
#'
#' @usage
...
#' 
#' -   Create a new global secondary index on the table. After the index begins backfilling, you can use [`update_table`][dynamodb_update_table] to perform other operations.
#' 
#' [`update_table`][dynamodb_update_table] is an asynchronous operation; while it's executing, the table status changes from `ACTIVE` to `UPDATING`. While it's `UPDATING`, you can't issue another [`update_table`][dynamodb_update_table] request. When the table returns to the `ACTIVE` state, the [`update_table`][dynamodb_update_table] operation is complete.
#'
#' @usage
