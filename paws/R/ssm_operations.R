...
#'   Tags, AlarmConfiguration, AssociationDispatchAssumeRole)
#'
...
#' 
#' A resource data sync is an asynchronous operation that returns immediately. After a successful initial sync is completed, the system continuously syncs data. To check the status of a sync, use the [`list_resource_data_sync`][ssm_list_resource_data_sync].
#' 
...
#' 
#' Before you delete the document, we recommend that you use [`delete_association`][ssm_delete_association] to disassociate all managed nodes that are associated with the document.
#'
#' @usage
...
#' 
#' -   This operation uses an *eventual consistency model*, which means the system can take a few minutes to complete this operation. If you delete an OpsItem and immediately call, for example, [`get_ops_item`][ssm_get_ops_item], the deleted OpsItem might still appear in the response.
#' 
...
#' Lists all patches eligible to be included in a patch baseline.
#' 
#' Currently, [`describe_available_patches`][ssm_describe_available_patches] supports only the Amazon Linux 1, Amazon Linux 2, and Windows Server operating systems.
#'
#' @usage
...
#' 
#' If you change the KMS key alias for the KMS key used to encrypt a parameter, then you must also update the key alias the parameter uses to reference KMS. Otherwise, [`describe_parameters`][ssm_describe_parameters] retrieves whatever the original key alias was referencing.
#'
#' @usage
...
#' 
#' [`get_command_invocation`][ssm_get_command_invocation] only gives the execution status of a plugin in a document. To get the command execution status on a specific managed node, use [`list_command_invocations`][ssm_list_command_invocations]. To get the command execution status across managed nodes, use [`list_commands`][ssm_list_commands].
#'
#' @usage
...
#' For maintenance window tasks without a specified target, you can't supply values for `--max-errors` and `--max-concurrency`. Instead, the system inserts a placeholder value of `1`, which may be reported in the response to this command. These values don't affect the running of your task and can be ignored.
#' 
#' To retrieve a list of tasks in a maintenance window, instead use the [`describe_maintenance_window_tasks`][ssm_describe_maintenance_window_tasks] command.
#'
#' @usage
...
#' Parameter names can't contain spaces. The service removes any spaces specified for the beginning or end of a parameter name. If the specified name for a parameter contains spaces between characters, the request fails with a `ValidationException` error.
#' 
#' If you change the KMS key alias for the KMS key used to encrypt a parameter, then you must also update the key alias the parameter uses to reference KMS. Otherwise, [`get_parameter_history`][ssm_get_parameter_history] retrieves whatever the original key alias was referencing.
#'
#' @usage
...
#' 
#' Services map a `SettingId` object to a setting value. Amazon Web Services services teams define the default value for a `SettingId`. You can't create a new `SettingId`, but you can overwrite the default value if you have the `ssm:UpdateServiceSetting` permission for the setting. Use the [`update_service_setting`][ssm_update_service_setting] API operation to change the default setting. Or use the [`reset_service_setting`][ssm_reset_service_setting] to change the value back to the original value defined by the Amazon Web Services service team.
#' 
...
#' 
#' The number of sync configurations might be too large to return using a single call to [`list_resource_data_sync`][ssm_list_resource_data_sync]. You can limit the number of sync configurations returned by using the `MaxResults` parameter. To determine whether there are more sync configurations to list, check the value of `NextToken` in the output. If there are more sync configurations to list, you can request them by specifying the `NextToken` returned in the call to the parameter of a subsequent call.
#'
#' @usage
...
#' Returns a list of the tags assigned to the specified resource.
#' 
#' For information about the ID format for each supported resource type, see [`add_tags_to_resource`][ssm_add_tags_to_resource].
#'
#' @usage
...
#' `ServiceSetting` is an account-level setting for an Amazon Web Services service. This setting defines how a user interacts with or uses a service or a feature of a service. For example, if an Amazon Web Services service charges money to the account based on feature or service usage, then the Amazon Web Services service team might create a default setting of "false". This means the user can't use this feature unless they change the setting to "true" and intentionally opt in for a paid feature.
#' 
#' Services map a `SettingId` object to a setting value. Amazon Web Services services teams define the default value for a `SettingId`. You can't create a new `SettingId`, but you can overwrite the default value if you have the `ssm:UpdateServiceSetting` permission for the setting. Use the [`get_service_setting`][ssm_get_service_setting] API operation to view the current value. Use the [`update_service_setting`][ssm_update_service_setting] API operation to change the default setting.
#' 
...
#' 
#' In order to call this API operation, a user, group, or role must be granted permission to call the [`describe_association`][ssm_describe_association] API operation. If you don't have permission to call [`describe_association`][ssm_describe_association], then you receive the following error: `An error occurred (AccessDeniedException) when calling the UpdateAssociation operation: User: <user_arn> isn\'t authorized to perform: ssm:DescribeAssociation on resource: <resource_arn>`
#' 
...
#'   Duration, TargetMaps, AlarmConfiguration, AssociationDispatchAssumeRole)
#'
...
#' 
#' [`update_association_status`][ssm_update_association_status] is primarily used by the Amazon Web Services Systems Manager Agent (SSM Agent) to report status updates about your associations and is only used for associations created with the `InstanceId` legacy parameter.
#'
#' @usage
...
#' One or more targets must be specified for maintenance window Run Command-type tasks. Depending on the task, targets are optional for other maintenance window task types (Automation, Lambda, and Step Functions). For more information about running tasks that don't specify targets, see [Registering maintenance window tasks without targets](https://docs.aws.amazon.com/systems-manager/latest/userguide/maintenance-windows-targetless-tasks.html) in the *Amazon Web Services Systems Manager User Guide*.
#' 
#' If the value for a parameter in [`update_maintenance_window_task`][ssm_update_maintenance_window_task] is null, then the corresponding field isn't modified. If you set `Replace` to true, then all fields required by the [`register_task_with_maintenance_window`][ssm_register_task_with_maintenance_window] operation are required for this request. Optional fields that aren't specified are set to null.
#' 
...
#' `ServiceSetting` is an account-level setting for an Amazon Web Services service. This setting defines how a user interacts with or uses a service or a feature of a service. For example, if an Amazon Web Services service charges money to the account based on feature or service usage, then the Amazon Web Services service team might create a default setting of "false". This means the user can't use this feature unless they change the setting to "true" and intentionally opt in for a paid feature.
#' 
#' Services map a `SettingId` object to a setting value. Amazon Web Services services teams define the default value for a `SettingId`. You can't create a new `SettingId`, but you can overwrite the default value if you have the `ssm:UpdateServiceSetting` permission for the setting. Use the [`get_service_setting`][ssm_get_service_setting] API operation to view the current value. Or, use the [`reset_service_setting`][ssm_reset_service_setting] to change the value back to the original value defined by the Amazon Web Services service team.
#' 
