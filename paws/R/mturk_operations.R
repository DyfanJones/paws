...
#' 
#' A successful request for the [`accept_qualification_request`][mturk_accept_qualification_request] operation returns with no errors and an empty body.
#'
#' @usage
...
#' 
#' You can only assign a Qualification of a Qualification type that you created (using the [`create_qualification_type`][mturk_create_qualification_type] operation).
#' 
#' Note: [`associate_qualification_with_worker`][mturk_associate_qualification_with_worker] does not affect any pending Qualification requests for the Qualification by the Worker. If you assign a Qualification to a Worker, then later grant a Qualification request made by the Worker, the granting of the request may modify the Qualification score. To resolve a pending Qualification request without affecting the Qualification the Worker already has, reject the request with the [`reject_qualification_request`][mturk_reject_qualification_request] operation.
#'
#' @usage
...
#' 
#' This operation allows you to specify a new HIT by passing in values for the properties of the HIT, such as its title, reward amount and number of assignments. When you pass these values to [`create_hit`][mturk_create_hit], a new HIT is created for you, with a new `HITTypeID`. The HITTypeID can be used to create additional HITs in the future without needing to specify common parameters such as the title, description and reward amount each time.
#' 
#' An alternative way to create HITs is to first generate a HITTypeID using the [`create_hit_type`][mturk_create_hit_type] operation and then call the [`create_hit_with_hit_type`][mturk_create_hit_with_hit_type] operation. This is the recommended best practice for Requesters who are creating large numbers of HITs.
#' 
...
#' 
#' This is an alternative way to create HITs from the [`create_hit`][mturk_create_hit] operation. This is the recommended best practice for Requesters who are creating large numbers of HITs.
#' 
...
#' 
#' To get a Worker's Qualification, you must know the Worker's ID. The Worker's ID is included in the assignment data returned by the [`list_assignments_for_hit`][mturk_list_assignments_for_hit] operation.
#' 
