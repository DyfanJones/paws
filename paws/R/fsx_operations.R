...
#' 
#' The following actions are related to [`create_and_attach_s3_access_point`][fsx_create_and_attach_s3_access_point]:
#' 
...
#' 
#' [`create_data_repository_association`][fsx_create_data_repository_association] isn't supported on Amazon File Cache resources. To create a DRA on Amazon File Cache, use the [`create_file_cache`][fsx_create_file_cache] operation.
#'
#' @usage
...
#' Creates a new Amazon File Cache resource.
#' 
#' You can use this operation with a client request token in the request that Amazon File Cache uses to ensure idempotent creation. If a cache with the specified client request token exists and the parameters match, [`create_file_cache`][fsx_create_file_cache] returns the description of the existing cache. If a cache with the specified client request token exists and the parameters don't match, this call returns `IncompatibleParameterError`. If a file cache with the specified client request token doesn't exist, [`create_file_cache`][fsx_create_file_cache] does the following:
#' 
#' -   Creates a new, empty Amazon File Cache resource with an assigned ID, and an initial lifecycle state of `CREATING`.
#' 
#' -   Returns the description of the cache in JSON format.
#' 
#' The [`create_file_cache`][fsx_create_file_cache] call returns while the cache's lifecycle state is still `CREATING`. You can check the cache creation status by calling the [`describe_file_caches`][fsx_describe_file_caches] operation, which returns the cache state along with other information.
#'
#' @usage
...
#' -   Amazon FSx for Windows File Server
#' 
#' This operation requires a client request token in the request that Amazon FSx uses to ensure idempotent creation. This means that calling the operation multiple times with the same client request token has no effect. By using the idempotent operation, you can retry a [`create_file_system`][fsx_create_file_system] operation without the risk of creating an extra file system. This approach can be useful when an initial call fails in a way that makes it unclear whether a file system was created. Examples are if a transport level timeout occurred, or your connection was reset. If you use the same client request token and the initial call created a file system, the client receives success as long as the parameters are the same.
#' 
#' If a file system with the specified client request token exists and the parameters match, [`create_file_system`][fsx_create_file_system] returns the description of the existing file system. If a file system with the specified client request token exists and the parameters don't match, this call returns `IncompatibleParameterError`. If a file system with the specified client request token doesn't exist, [`create_file_system`][fsx_create_file_system] does the following:
#' 
#' -   Creates a new, empty Amazon FSx file system with an assigned ID, and an initial lifecycle state of `CREATING`.
#' 
#' -   Returns the description of the file system in JSON format.
#' 
#' The [`create_file_system`][fsx_create_file_system] call returns while the file system's lifecycle state is still `CREATING`. You can check the file-system creation status by calling the [`describe_file_systems`][fsx_describe_file_systems] operation, which returns the file system state along with other information.
#'
#' @usage
...
#' 
#' The [`delete_backup`][fsx_delete_backup] call returns instantly. The backup won't show up in later [`describe_backups`][fsx_describe_backups] calls.
#' 
...
#' 
#' The [`delete_file_cache`][fsx_delete_file_cache] operation returns while the cache has the `DELETING` status. You can check the cache deletion status by calling the [`describe_file_caches`][fsx_describe_file_caches] operation, which returns a list of caches in your account. If you pass the cache ID for a deleted cache, the [`describe_file_caches`][fsx_describe_file_caches] operation returns a `FileCacheNotFound` error.
#' 
...
#' 
#' To delete an Amazon FSx for NetApp ONTAP file system, first delete all the volumes and storage virtual machines (SVMs) on the file system. Then provide a `FileSystemId` value to the [`delete_file_system`][fsx_delete_file_system] operation.
#' 
...
#' 
#' To delete an Amazon FSx for Lustre file system, first [unmount](https://docs.aws.amazon.com/fsx/latest/LustreGuide/unmounting-fs.html) it from every connected Amazon EC2 instance, then provide a `FileSystemId` value to the [`delete_file_system`][fsx_delete_file_system] operation. By default, Amazon FSx will not take a final backup when the [`delete_file_system`][fsx_delete_file_system] operation is invoked. On file systems not linked to an Amazon S3 bucket, set `SkipFinalBackup` to `false` to take a final backup of the file system you are deleting. Backups cannot be enabled on S3-linked file systems. To ensure all of your data is written back to S3 before deleting your file system, you can either monitor for the [AgeOfOldestQueuedMessage](https://docs.aws.amazon.com/fsx/latest/LustreGuide/monitoring-cloudwatch.html#auto-import-export-metrics) metric to be zero (if using automatic export) or you can run an [export data repository task](https://docs.aws.amazon.com/fsx/latest/LustreGuide/export-data-repo-task-dra.html). If you have automatic export enabled and want to use an export data repository task, you have to disable automatic export before executing the export data repository task.
#' 
#' The [`delete_file_system`][fsx_delete_file_system] operation returns while the file system has the `DELETING` status. You can check the file system deletion status by calling the [`describe_file_systems`][fsx_describe_file_systems] operation, which returns a list of file systems in your account. If you pass the file system ID for a deleted file system, the [`describe_file_systems`][fsx_describe_file_systems] operation returns a `FileSystemNotFound` error.
#' 
...
#' 
#' The [`delete_snapshot`][fsx_delete_snapshot] operation returns instantly. The snapshot appears with the lifecycle status of `DELETING` until the deletion is complete.
#'
#' @usage
...
#' 
#' -   The order of the backups returned in the response of one [`describe_backups`][fsx_describe_backups] call and the order of the backups returned across the responses of a multi-call iteration is unspecified.
#'
#' @usage
...
#' 
#' This operation is used in an iterative process to retrieve a list of your cache descriptions. [`describe_file_caches`][fsx_describe_file_caches] is called first without a `NextToken`value. Then the operation continues to be called with the `NextToken` parameter set to the value of the last `NextToken` value until a response has no `NextToken`.
#' 
#' When using this operation, keep the following in mind:
#' 
#' -   The implementation might return fewer than `MaxResults` cache descriptions while still including a `NextToken` value.
#' 
#' -   The order of caches returned in the response of one [`describe_file_caches`][fsx_describe_file_caches] call and the order of caches returned across the responses of a multicall iteration is unspecified.
#'
#' @usage
...
#' 
#' This operation is used in an iterative process to retrieve a list of your file system descriptions. [`describe_file_systems`][fsx_describe_file_systems] is called first without a `NextToken`value. Then the operation continues to be called with the `NextToken` parameter set to the value of the last `NextToken` value until a response has no `NextToken`.
#' 
#' When using this operation, keep the following in mind:
#' 
#' -   The implementation might return fewer than `MaxResults` file system descriptions while still including a `NextToken` value.
#' 
#' -   The order of file systems returned in the response of one [`describe_file_systems`][fsx_describe_file_systems] call and the order of file systems returned across the responses of a multicall iteration is unspecified.
#'
#' @usage
...
#' 
#' Use this operation in an iterative process to retrieve a list of your snapshots. [`describe_snapshots`][fsx_describe_snapshots] is called first without a `NextToken` value. Then the operation continues to be called with the `NextToken` parameter set to the value of the last `NextToken` value until a response has no `NextToken` value.
#' 
#' When using this operation, keep the following in mind:
#' 
...
#' 
#' -   The order of snapshots returned in the response of one [`describe_snapshots`][fsx_describe_snapshots] call and the order of backups returned across the responses of a multi-call iteration is unspecified.
#'
#' @usage
...
#' 
#' This action is used in an iterative process to retrieve a list of your tags. [`list_tags_for_resource`][fsx_list_tags_for_resource] is called first without a `NextToken`value. Then the action continues to be called with the `NextToken` parameter set to the value of the last `NextToken` value until a response has no `NextToken`.
#' 
#' When using this action, keep the following in mind:
#' 
#' -   The implementation might return fewer than `MaxResults` file system descriptions while still including a `NextToken` value.
#' 
#' -   The order of tags returned in the response of one [`list_tags_for_resource`][fsx_list_tags_for_resource] call and the order of tags returned across the responses of a multi-call iteration is unspecified.
#'
#' @usage
