...
#' 
#' This is an asynchronous operation that immediately returns. The initial status of the Firehose stream is `CREATING`. After the Firehose stream is created, its status is `ACTIVE` and it now accepts data. If the Firehose stream creation fails, the status transitions to `CREATING_FAILED`. Attempts to send data to a delivery stream that is not in the `ACTIVE` state cause an exception. To check the state of a Firehose stream, use [`describe_delivery_stream`][firehose_describe_delivery_stream].
#' 
#' If the status of a Firehose stream is `CREATING_FAILED`, this status doesn't change, and you can't invoke [`create_delivery_stream`][firehose_create_delivery_stream] again on it. However, you can invoke the [`delete_delivery_stream`][firehose_delete_delivery_stream] operation to delete it.
#' 
#' A Firehose stream can be configured to receive records directly from providers using [`put_record`][firehose_put_record] or [`put_record_batch`][firehose_put_record_batch], or it can be configured to use an existing Kinesis stream as its source. To specify a Kinesis data stream as input, set the `DeliveryStreamType` parameter to `KinesisStreamAsSource`, and provide the Kinesis stream Amazon Resource Name (ARN) and role ARN in the `KinesisStreamSourceConfiguration` parameter.
#' 
#' To create a Firehose stream with server-side encryption (SSE) enabled, include DeliveryStreamEncryptionConfigurationInput in your request. This is optional. You can also invoke [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] to turn on SSE for an existing Firehose stream that doesn't have SSE enabled.
#' 
...
#' Deletes a Firehose stream and its data.
#' 
#' You can delete a Firehose stream only if it is in one of the following states: `ACTIVE`, `DELETING`, `CREATING_FAILED`, or `DELETING_FAILED`. You can't delete a Firehose stream that is in the `CREATING` state. To check the state of a Firehose stream, use [`describe_delivery_stream`][firehose_describe_delivery_stream].
#' 
...
#' 
#' If the status of a Firehose stream is `CREATING_FAILED`, this status doesn't change, and you can't invoke [`create_delivery_stream`][firehose_create_delivery_stream] again on it. However, you can invoke the [`delete_delivery_stream`][firehose_delete_delivery_stream] operation to delete it. If the status is `DELETING_FAILED`, you can force deletion by invoking [`delete_delivery_stream`][firehose_delete_delivery_stream] again but with DeleteDeliveryStreamInput$AllowForceDelete set to true.
#'
#' @usage
...
#' Lists your Firehose streams in alphabetical order of their names.
#' 
#' The number of Firehose streams might be too large to return using a single call to [`list_delivery_streams`][firehose_list_delivery_streams]. You can limit the number of Firehose streams returned, using the `Limit` parameter. To determine whether there are more delivery streams to list, check the value of `HasMoreDeliveryStreams` in the output. If there are more Firehose streams to list, you can request them by calling this operation again and setting the `ExclusiveStartDeliveryStreamName` parameter to the name of the last Firehose stream returned in the last call.
#'
#' @usage
...
#' 
#' By default, each Firehose stream can take in up to 2,000 transactions per second, 5,000 records per second, or 5 MB per second. If you use [`put_record`][firehose_put_record] and [`put_record_batch`][firehose_put_record_batch], the limits are an aggregate across these two operations for each Firehose stream. For more information about limits and how to request an increase, see [Amazon Firehose Limits](https://docs.aws.amazon.com/firehose/latest/dev/limits.html).
#' 
...
#' 
#' The [`put_record`][firehose_put_record] operation returns a `RecordId`, which is a unique string assigned to each record. Producer applications can use this ID for purposes such as auditability and investigation.
#' 
#' If the [`put_record`][firehose_put_record] operation throws a `ServiceUnavailableException`, the API is automatically reinvoked (retried) 3 times. If the exception persists, it is possible that the throughput limits have been exceeded for the Firehose stream.
#' 
...
#' 
#' Each [`put_record_batch`][firehose_put_record_batch] request supports up to 500 records. Each record in the request can be as large as 1,000 KB (before base64 encoding), up to a limit of 4 MB for the entire request. These limits cannot be changed.
#' 
...
#' Firehose buffers records before delivering them to the destination. To disambiguate the data blobs at the destination, a common solution is to use delimiters in the data, such as a newline (`\\n`) or some other character unique within the data. This allows the consumer application to parse individual data items when reading the data from the destination.
#' 
#' The [`put_record_batch`][firehose_put_record_batch] response includes a count of failed records, `FailedPutCount`, and an array of responses, `RequestResponses`. Even if the [`put_record_batch`][firehose_put_record_batch] call succeeds, the value of `FailedPutCount` may be greater than 0, indicating that there are records for which the operation didn't succeed. Each entry in the `RequestResponses` array provides additional information about the processed record. It directly correlates with a record in the request array using the same ordering, from the top to the bottom. The response array always includes the same number of records as the request array. `RequestResponses` includes both successfully and unsuccessfully processed records. Firehose tries to process all records in each [`put_record_batch`][firehose_put_record_batch] request. A single record failure does not stop the processing of subsequent records.
#' 
...
#' 
#' If [`put_record_batch`][firehose_put_record_batch] throws `ServiceUnavailableException`, the API is automatically reinvoked (retried) 3 times. If the exception persists, it is possible that the throughput limits have been exceeded for the Firehose stream.
#' 
#' Re-invoking the Put API operations (for example, PutRecord and PutRecordBatch) can result in data duplicates. For larger data assets, allow for a longer time out before retrying Put API operations.
#' 
...
#' 
#' To check the encryption status of a Firehose stream, use [`describe_delivery_stream`][firehose_describe_delivery_stream].
#' 
...
#' 
#' For the KMS grant creation to be successful, the Firehose API operations [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] and [`create_delivery_stream`][firehose_create_delivery_stream] should not be called with session credentials that are more than 6 hours old.
#' 
...
#' 
#' The [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] and [`stop_delivery_stream_encryption`][firehose_stop_delivery_stream_encryption] operations have a combined limit of 25 calls per Firehose stream per 24 hours. For example, you reach the limit if you call [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] 13 times and [`stop_delivery_stream_encryption`][firehose_stop_delivery_stream_encryption] 12 times for the same Firehose stream in a 24-hour period.
#'
#' @usage
...
#' 
#' To check the encryption state of a Firehose stream, use [`describe_delivery_stream`][firehose_describe_delivery_stream].
#' 
#' If SSE is enabled using a customer managed CMK and then you invoke [`stop_delivery_stream_encryption`][firehose_stop_delivery_stream_encryption], Firehose schedules the related KMS grant for retirement and then retires it after it ensures that it is finished delivering records to the destination.
#' 
#' The [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] and [`stop_delivery_stream_encryption`][firehose_stop_delivery_stream_encryption] operations have a combined limit of 25 calls per Firehose stream per 24 hours. For example, you reach the limit if you call [`start_delivery_stream_encryption`][firehose_start_delivery_stream_encryption] 13 times and [`stop_delivery_stream_encryption`][firehose_stop_delivery_stream_encryption] 12 times for the same Firehose stream in a 24-hour period.
#'
#' @usage
...
#' 
#' Firehose uses `CurrentDeliveryStreamVersionId` to avoid race conditions and conflicting merges. This is a required field, and the service updates the configuration only if the existing configuration has a version ID that matches. After the update is applied successfully, the version ID is updated, and can be retrieved using [`describe_delivery_stream`][firehose_describe_delivery_stream]. Use the new version ID to set `CurrentDeliveryStreamVersionId` in the next call.
#'
#' @usage
