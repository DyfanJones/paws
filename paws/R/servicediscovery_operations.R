...
#' 
#' The [`create_public_dns_namespace`][servicediscovery_create_public_dns_namespace] API operation is not supported in the Amazon Web Services GovCloud (US) Regions.
#'
#' @usage
...
#' 
#' After you create the service, you can submit a [`register_instance`][servicediscovery_register_instance] request, and Cloud Map uses the values in the configuration to create the specified entities.
#' 
...
#' 
#' To get a list of operations that match specified criteria, see [`list_operations`][servicediscovery_list_operations].
#'
#' @usage
...
#' 
#' One [`register_instance`][servicediscovery_register_instance] request must complete before you can submit another request and specify the same service ID and instance ID.
#' 
#' For more information, see [`create_service`][servicediscovery_create_service].
#' 
...
#' 
#' -   If you omit any existing `DnsRecords` or `HealthCheckConfig` configurations from an [`update_service`][servicediscovery_update_service] request, the configurations are deleted from the service.
#' 
#' -   If you omit an existing `HealthCheckCustomConfig` configuration from an [`update_service`][servicediscovery_update_service] request, the configuration isn't deleted from the service.
#' 
#' You can't call [`update_service`][servicediscovery_update_service] and update settings in the following scenarios:
#' 
#' -   When the service is associated with an HTTP namespace
#' 
#' -   When the service is associated with a shared namespace and contains instances that were registered by Amazon Web Services accounts other than the account making the [`update_service`][servicediscovery_update_service] call
#' 
