...
#' 
#' You can copy a product to the same account or another account. You can copy a product to the same Region or another Region. If you copy a product to another account, you must first share the product in a portfolio using [`create_portfolio_share`][servicecatalog_create_portfolio_share].
#' 
#' This operation is performed asynchronously. To track the progress of the operation, use [`describe_copy_product_status`][servicecatalog_describe_copy_product_status].
#'
#' @usage
...
#' 
#' If the portfolio share with the specified account or organization node already exists, this action will have no effect and will not return an error. To update an existing share, you must use the [`update_portfolio_share`][servicecatalog_update_portfolio_share] API instead.
#' 
#' When you associate a principal with portfolio, a potential privilege escalation path may occur when that portfolio is then shared with other accounts. For a user in a recipient account who is *not* an Service Catalog Admin, but still has the ability to create Principals (Users/Groups/Roles), that user could create a role that matches a principal name association for the portfolio. Although this user may not know which principal names are associated through Service Catalog, they may be able to guess the user. If this potential escalation path is a concern, then Service Catalog recommends using `PrincipalType` as `IAM`. With this configuration, the `PrincipalARN` must already exist in the recipient account before it can be associated.
#'
#' @usage
...
#' 
#' To view the resource changes in the change set, use [`describe_provisioned_product_plan`][servicecatalog_describe_provisioned_product_plan]. To create or modify the provisioned product, use [`execute_provisioned_product_plan`][servicecatalog_execute_provisioned_product_plan].
#'
#' @usage
...
#' Gets information about the specified product.
#' 
#' Running this operation with administrator access results in a failure. [`describe_product_as_admin`][servicecatalog_describe_product_as_admin] should be used instead.
#'
#' @usage
...
#' Gets information about the specified request operation.
#' 
#' Use this operation after calling a request operation (for example, [`provision_product`][servicecatalog_provision_product], [`terminate_provisioned_product`][servicecatalog_terminate_provisioned_product], or [`update_provisioned_product`][servicecatalog_update_provisioned_product]).
#' 
#' If a provisioned product was transferred to a new owner using [`update_provisioned_product_properties`][servicecatalog_update_provisioned_product_properties], the new owner will be able to describe all past records for that product. The previous owner will no longer be able to describe the records, but will be able to use [`list_record_history`][servicecatalog_list_record_history] to see the product's history from when he was the owner.
#'
#' @usage
...
#' 
#' Note that a delegated administrator is not authorized to invoke [`disable_aws_organizations_access`][servicecatalog_disable_aws_organizations_access].
#' 
...
#' 
#' The `PrincipalType` and `PrincipalARN` must match the [`associate_principal_with_portfolio`][servicecatalog_associate_principal_with_portfolio] call request details. For example, to disassociate an association created with a `PrincipalARN` of `PrincipalType` IAM you must use the `PrincipalType` IAM when calling [`disassociate_principal_from_portfolio`][servicecatalog_disassociate_principal_from_portfolio].
#' 
...
#' 
#' Note that a delegated administrator is not authorized to invoke [`enable_aws_organizations_access`][servicecatalog_enable_aws_organizations_access].
#' 
...
#' 
#' When you import an existing CloudFormation stack into a portfolio, Service Catalog does not apply the product's associated constraints during the import process. Service Catalog applies the constraints after you call [`update_provisioned_product`][servicecatalog_update_provisioned_product] for the provisioned product.
#' 
...
#' Provisions the specified product.
#' 
#' A provisioned product is a resourced instance of a product. For example, provisioning a product that's based on an CloudFormation template launches an CloudFormation stack and its underlying resources. You can check the status of this request using [`describe_record`][servicecatalog_describe_record].
#' 
...
#' Lists the provisioned products that are available (not terminated).
#' 
#' To use additional filtering, see [`search_provisioned_products`][servicecatalog_search_provisioned_products].
#'
#' @usage
...
#' 
#' You can check the status of this request using [`describe_record`][servicecatalog_describe_record].
#'
#' @usage
...
#' 
#' The portfolio share cannot be updated if the [`create_portfolio_share`][servicecatalog_create_portfolio_share] operation is `IN_PROGRESS`, as the share is not available to recipient entities. In this case, you must wait for the portfolio share to be completed.
#' 
#' You must provide the `accountId` or organization node in the input, but not both.
#' 
#' If the portfolio is shared to both an external account and an organization node, and both shares need to be updated, you must invoke [`update_portfolio_share`][servicecatalog_update_portfolio_share] separately for each share type.
#' 
#' This API cannot be used for removing the portfolio share. You must use [`delete_portfolio_share`][servicecatalog_delete_portfolio_share] API for that action.
#' 
#' When you associate a principal with portfolio, a potential privilege escalation path may occur when that portfolio is then shared with other accounts. For a user in a recipient account who is *not* an Service Catalog Admin, but still has the ability to create Principals (Users/Groups/Roles), that user could create a role that matches a principal name association for the portfolio. Although this user may not know which principal names are associated through Service Catalog, they may be able to guess the user. If this potential escalation path is a concern, then Service Catalog recommends using `PrincipalType` as `IAM`. With this configuration, the `PrincipalARN` must already exist in the recipient account before it can be associated.
#'
#' @usage
