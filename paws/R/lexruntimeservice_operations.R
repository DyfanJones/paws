...
#' @param botName &#91;required&#93; The name of the bot that contains the session data.
#' @param botAlias &#91;required&#93; The alias in use for the bot that contains the session data.
#' @param userId &#91;required&#93; The ID of the client application user. Amazon Lex uses this to identify a user's conversation with your bot.
...
#' 
#' The [`post_content`][lexruntimeservice_post_content] operation supports audio input at 8kHz and 16kHz. You can use 8kHz audio to achieve higher speech recognition accuracy in telephone audio applications.
#' 
...
#' -   `slotToElict`
#' 
#' If you send the `recentIntentSummaryView` parameter in a [`put_session`][lexruntimeservice_put_session] request, the contents of the new summary view replaces the old summary view. For example, if a [`get_session`][lexruntimeservice_get_session] request returns three intents in the summary view and you call [`put_session`][lexruntimeservice_put_session] with one intent in the summary view, the next call to [`get_session`][lexruntimeservice_get_session] will only return one intent.
