...
#' 
#' If you need to pause the pipeline to investigate an issue with a component, such as a data source or script, call [`deactivate_pipeline`][datapipeline_deactivate_pipeline].
#' 
...
#' 
#' To resume a deactivated pipeline, use [`activate_pipeline`][datapipeline_activate_pipeline]. By default, the pipeline resumes from the last completed execution. Optionally, you can specify the date and time to resume the pipeline.
#'
#' @usage
...
#' 
#' Deleting a pipeline cannot be undone. You cannot query or restore a deleted pipeline. To temporarily pause a pipeline instead of deleting it, call [`set_status`][datapipeline_set_status] with the status set to `PAUSE` on individual components. Components that are paused by [`set_status`][datapipeline_set_status] can be resumed.
#'
#' @usage
...
#' 
#' To retrieve the full pipeline definition instead of metadata about the pipeline, call [`get_pipeline_definition`][datapipeline_get_pipeline_definition].
#'
#' @usage
...
#' 
#' If tasks are ready in the work queue, [`poll_for_task`][datapipeline_poll_for_task] returns a response immediately. If no tasks are available in the queue, [`poll_for_task`][datapipeline_poll_for_task] uses long-polling and holds on to a poll connection for up to a 90 seconds, during which time the first newly scheduled task is handed to the task runner. To accomodate this, set the socket timeout in your task runner to 90 seconds. The task runner should not call [`poll_for_task`][datapipeline_poll_for_task] again on the same `workerGroup` until it receives a response, and this can take up to 90 seconds.
#'
#' @usage
...
#' 
#' [`put_pipeline_definition`][datapipeline_put_pipeline_definition] also validates the configuration as it adds it to the pipeline. Changes to the pipeline are saved unless one of the following three validation errors exists in the pipeline.
#' 
...
#' 
#' Pipeline object definitions are passed to the [`put_pipeline_definition`][datapipeline_put_pipeline_definition] action and returned by the [`get_pipeline_definition`][datapipeline_get_pipeline_definition] action.
#'
#' @usage
