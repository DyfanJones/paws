...
#' 
#' This guide describes the IAM Access Analyzer operations that you can call programmatically. For general information about IAM Access Analyzer, see [Using Identity and Access Management Access Analyzer](https://docs.aws.amazon.com/IAM/latest/UserGuide/what-is-access-analyzer.html) in the **IAM User Guide**.
#'
#' @param
#' config
...
#'  \link[=accessanalyzer_create_service_linked_analyzer]{create_service_linked_analyzer} \tab Creates a service-linked analyzer managed by an Amazon Web Services service\cr
...
#'  \link[=accessanalyzer_delete_service_linked_analyzer]{delete_service_linked_analyzer} \tab Deletes a service-linked analyzer\cr
...
.accessanalyzer$metadata <- list(
  ...
  endpoints = list("^(us|eu|ap|sa|ca|me|af|il|mx)\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.amazonaws.com", global = FALSE), "^cn\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.amazonaws.com.cn", global = FALSE), "^us\\-gov\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.amazonaws.com", global = FALSE), "^us\\-iso\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.c2s.ic.gov", global = FALSE), "^us\\-isob\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.sc2s.sgov.gov", global = FALSE), "^eu\\-isoe\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.cloud.adc-e.uk", global = FALSE), "^us\\-isof\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.csp.hci.ic.gov", global = FALSE), "^eusc\\-(de)\\-\\w+\\-\\d+$" = list(endpoint = "access-analyzer.{region}.amazonaws.eu", global = FALSE)),
