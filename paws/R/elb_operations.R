...
#' 
#' Each tag consists of a key and an optional value. If a tag with the same key is already associated with the load balancer, [`add_tags`][elb_add_tags] updates its value.
#' 
...
#' 
#' For more information, see [Security Groups for Load Balancers in a VPC](https://docs.aws.amazon.com/elasticloadbalancing/latest/classic/#elb-vpc-security-groups) in the *Classic Load Balancers Guide*.
#'
#' @usage
...
#' 
#' This policy is similar to the policy created by [`create_lb_cookie_stickiness_policy`][elb_create_lb_cookie_stickiness_policy], except that the lifetime of the special Elastic Load Balancing cookie, `AWSELB`, follows the lifetime of the application-generated cookie specified in the policy configuration. The load balancer only inserts a new stickiness cookie when the application response includes a new application cookie.
#' 
...
#' Creates a Classic Load Balancer.
#' 
#' You can add listeners, security groups, subnets, and tags when you create your load balancer, or you can add them later using [`create_load_balancer_listeners`][elb_create_load_balancer_listeners], [`apply_security_groups_to_load_balancer`][elb_apply_security_groups_to_load_balancer], [`attach_load_balancer_to_subnets`][elb_attach_load_balancer_to_subnets], and [`add_tags`][elb_add_tags].
#' 
#' To describe your current load balancers, see [`describe_load_balancers`][elb_describe_load_balancers]. When you are finished with a load balancer, you can delete it using [`delete_load_balancer`][elb_delete_load_balancer].
#' 
...
#' 
#' If the load balancer does not exist or has already been deleted, the call to [`delete_load_balancer`][elb_delete_load_balancer] still succeeds.
#'
#' @usage
...
#' 
#' You can use [`describe_load_balancers`][elb_describe_load_balancers] to verify that the instance is deregistered from the load balancer.
#' 
...
#' 
#' You can use [`create_load_balancer_policy`][elb_create_load_balancer_policy] to create a policy configuration for any of these policy types. Then, depending on the policy type, use either [`set_load_balancer_policies_of_listener`][elb_set_load_balancer_policies_of_listener] or [`set_load_balancer_policies_for_backend_server`][elb_set_load_balancer_policies_for_backend_server] to set the policy.
#'
#' @usage
...
#' 
#' For load balancers in a non-default VPC, use [`detach_load_balancer_from_subnets`][elb_detach_load_balancer_from_subnets].
#' 
#' There must be at least one Availability Zone registered with a load balancer at all times. After an Availability Zone is removed, all instances registered with the load balancer that are in the removed Availability Zone go into the `OutOfService` state. Then, the load balancer attempts to equally balance the traffic among its remaining Availability Zones.
#' 
#' For more information, see [Add or Remove Availability Zones](https://docs.aws.amazon.com/elasticloadbalancing/latest/classic/) in the *Classic Load Balancers Guide*.
#'
#' @usage
...
#' 
#' For load balancers in a non-default VPC, use [`attach_load_balancer_to_subnets`][elb_attach_load_balancer_to_subnets].
#' 
#' The load balancer evenly distributes requests across all its registered Availability Zones that contain instances. For more information, see [Add or Remove Availability Zones](https://docs.aws.amazon.com/elasticloadbalancing/latest/classic/) in the *Classic Load Balancers Guide*.
#'
#' @usage
...
#' 
#' Note that `RegisterInstanceWithLoadBalancer` completes when the request has been registered. Instance registration takes a little time to complete. To check the state of the registered instances, use [`describe_load_balancers`][elb_describe_load_balancers] or [`describe_instance_health`][elb_describe_instance_health].
#' 
#' After the instance is registered, it starts receiving traffic and requests from the load balancer. Any instance that is not in one of the Availability Zones registered for the load balancer is moved to the `OutOfService` state. If an Availability Zone is added to the load balancer later, any instances registered with the load balancer move to the `InService` state.
#' 
#' To deregister instances from a load balancer, use [`deregister_instances_from_load_balancer`][elb_deregister_instances_from_load_balancer].
#' 
#' For more information, see [Register or De-Register EC2 Instances](https://docs.aws.amazon.com/elasticloadbalancing/latest/classic/elb-deregister-register-instances.html) in the *Classic Load Balancers Guide*.
#'
#' @usage
...
#' 
#' Each time you use [`set_load_balancer_policies_for_backend_server`][elb_set_load_balancer_policies_for_backend_server] to enable the policies, use the `PolicyNames` parameter to list the policies that you want to enable.
#' 
#' You can use [`describe_load_balancers`][elb_describe_load_balancers] or [`describe_load_balancer_policies`][elb_describe_load_balancer_policies] to verify that the policy is associated with the EC2 instance.
#' 
...
#' 
#' To enable back-end server authentication, use [`set_load_balancer_policies_for_backend_server`][elb_set_load_balancer_policies_for_backend_server].
#' 
