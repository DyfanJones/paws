...
#' 
#' [`compare_faces`][rekognition_compare_faces] also returns an array of faces that don't match the source image. For each face, it returns a bounding box, confidence value, landmarks, pose details, and quality. The response also returns information about the face in the source image, including the bounding box of the face and confidence value.
#' 
#' The `QualityFilter` input parameter allows you to filter out detected faces that don’t meet a required quality bar. The quality bar is based on a variety of common use cases. Use `QualityFilter` to set the quality bar by specifying `LOW`, `MEDIUM`, or `HIGH`. If you do not want to filter detected faces, specify `NONE`. The default value is `NONE`.
#' 
#' If the image doesn't contain Exif metadata, [`compare_faces`][rekognition_compare_faces] returns orientation information for the source and target images. Use these values to display the images with the correct image orientation.
#' 
#' If no faces are detected in the source or target images, [`compare_faces`][rekognition_compare_faces] returns an `InvalidParameterException` error.
#' 
...
#' 
#' To copy a model version to a different AWS account, you need to create a resource-based policy known as a *project policy*. You attach the project policy to the source project by calling [`put_project_policy`][rekognition_put_project_policy]. The project policy gives permission to copy the model version from a trusting AWS account to a trusted account.
#' 
...
#' 
#' Copying a model version takes a while to complete. To get the current status, call [`describe_project_versions`][rekognition_describe_project_versions] and check the value of `Status` in the ProjectVersionDescription object. The copy operation has finished when the value of `Status` is `COPYING_COMPLETED`.
#' 
...
#' 
#' For example, you might create collections, one for each of your application users. A user can then index faces using the [`index_faces`][rekognition_index_faces] operation and persist results in a specific collection. Then, a user can search the collection for faces in the user-specific container.
#' 
...
#' 
#' The response from [`create_dataset`][rekognition_create_dataset] is the Amazon Resource Name (ARN) for the dataset. Creating a dataset takes a while to complete. Use [`describe_dataset`][rekognition_describe_dataset] to check the current status. The dataset created successfully if the value of `Status` is `CREATE_COMPLETE`.
#' 
#' To check if any non-terminal errors occurred, call [`list_dataset_entries`][rekognition_list_dataset_entries] and check for the presence of `errors` lists in the JSON Lines.
#' 
...
#' 
#' The FeatureConfig operation argument allows you to configure specific model or adapter settings. You can provide a description to the project version by using the VersionDescription argment. Training can take a while to complete. You can get the current status by calling [`describe_project_versions`][rekognition_describe_project_versions]. Training completed successfully if the value of the `Status` field is `TRAINING_COMPLETED`. Once training has successfully completed, call [`describe_project_versions`][rekognition_describe_project_versions] to get the training results and evaluate the model.
#' 
...
#' 
#' Uses a `ClientToken`, an idempotency token that ensures a call to [`create_user`][rekognition_create_user] completes only once. If the value is not supplied, the AWS SDK generates an idempotency token for the requests. This prevents retries after a network error results from making multiple [`create_user`][rekognition_create_user] calls.
#'
#' @usage
...
#' 
#' [`delete_project`][rekognition_delete_project] is an asynchronous operation. To check if the project is deleted, call [`describe_projects`][rekognition_describe_projects]. The project is deleted when the project no longer appears in the response. Be aware that deleting a given project will also delete any `ProjectPolicies` associated with that project.
#' 
...
#' 
#' To get a list of project policies attached to a project, call [`list_project_policies`][rekognition_list_project_policies]. To attach a project policy to a project, call [`put_project_policy`][rekognition_put_project_policy].
#' 
...
#' 
#' You can't delete a project version if it is running or if it is training. To check the status of a project version, use the Status field returned from [`describe_project_versions`][rekognition_describe_project_versions]. To stop a project version call [`stop_project_version`][rekognition_stop_project_version]. If the project version is training, wait until it finishes.
#' 
...
#' 
#' To filter labels that are returned, specify a value for `MinConfidence`. `DetectCustomLabelsLabels` only returns labels with a confidence that's higher than the specified value. The value of `MinConfidence` maps to the assumed threshold values created during training. For more information, see *Assumed threshold* in the Amazon Rekognition Custom Labels Developer Guide. Amazon Rekognition Custom Labels metrics expresses an assumed threshold as a floating point value between 0-1. The range of `MinConfidence` normalizes the threshold value to a percentage value (0-100). Confidence responses from [`detect_custom_labels`][rekognition_detect_custom_labels] are also returned as a percentage. You can use `MinConfidence` to change the precision and recall or your model. For more information, see *Analyzing an image* in the Amazon Rekognition Custom Labels Developer Guide.
#' 
#' If you don't specify a value for `MinConfidence`, [`detect_custom_labels`][rekognition_detect_custom_labels] returns labels based on the assumed threshold of each label.
#' 
...
#' 
#' If the object detected is a person, the operation doesn't provide the same facial details that the [`detect_faces`][rekognition_detect_faces] operation provides.
#' 
...
#' 
#' To filter images, use the labels returned by [`detect_moderation_labels`][rekognition_detect_moderation_labels] to determine which types of content are appropriate.
#' 
...
#' 
#' [`detect_protective_equipment`][rekognition_detect_protective_equipment] detects PPE worn by up to 15 persons detected in an image.
#' 
...
#' 
#' The [`detect_text`][rekognition_detect_text] operation returns text in an array of TextDetection elements, `TextDetections`. Each `TextDetection` element provides information about a single word or line of text that was detected in the image.
#' 
#' A word is one or more script characters that are not separated by spaces. [`detect_text`][rekognition_detect_text] can detect up to 100 words in an image.
#' 
#' A line is a string of equally spaced words. A line isn't necessarily a complete sentence. For example, a driver's license number is detected as a line. A line ends when there is no aligned text after it. Also, a line ends when there is a large gap between words, relative to the length of the words. This means, depending on the gap between words, Amazon Rekognition may detect multiple lines in text aligned in the same direction. Periods don't represent the end of a line. If a sentence spans multiple lines, the [`detect_text`][rekognition_detect_text] operation returns multiple lines.
#' 
...
#' 
#' Distributing a dataset takes a while to complete. To check the status call [`describe_dataset`][rekognition_describe_dataset]. The operation is complete when the `Status` field for the training dataset and the test dataset is `UPDATE_COMPLETE`. If the dataset split fails, the value of `Status` is `UPDATE_FAILED`.
#' 
...
#' 
#' Celebrity recognition in a video is an asynchronous operation. Analysis is started by a call to [`start_celebrity_recognition`][rekognition_start_celebrity_recognition] which returns a job identifier (`JobId`).
#' 
#' When the celebrity recognition operation finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_celebrity_recognition`][rekognition_start_celebrity_recognition]. To get the results of the celebrity recognition analysis, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call `GetCelebrityDetection` and pass the job identifier (`JobId`) from the initial call to `StartCelebrityDetection`.
#' 
#' For more information, see Working With Stored Videos in the Amazon Rekognition Developer Guide.
#' 
#' [`get_celebrity_recognition`][rekognition_get_celebrity_recognition] returns detected celebrities and the time(s) they are detected in an array (`Celebrities`) of CelebrityRecognition objects. Each `CelebrityRecognition` contains information about the celebrity in a CelebrityDetail object and the time, `Timestamp`, the celebrity was detected. This CelebrityDetail object stores information about the detected celebrity's face attributes, a face bounding box, known gender, the celebrity's name, and a confidence estimate.
#' 
#' [`get_celebrity_recognition`][rekognition_get_celebrity_recognition] only returns the default facial attributes (`BoundingBox`, `Confidence`, `Landmarks`, `Pose`, and `Quality`). The `BoundingBox` field only applies to the detected face instance. The other facial attributes listed in the `Face` object of the following response syntax are not returned. For more information, see FaceDetail in the Amazon Rekognition Developer Guide.
#' 
#' By default, the `Celebrities` array is sorted by time (milliseconds from the start of the video). You can also sort the array by celebrity by specifying the value `ID` in the `SortBy` input parameter.
#' 
#' The `CelebrityDetail` object includes the celebrity identifer and additional information urls. If you don't store the additional information urls, you can get them later by calling [`get_celebrity_info`][rekognition_get_celebrity_info] with the celebrity identifer.
#' 
#' No information is returned for faces not recognized as celebrities.
#' 
#' Use MaxResults parameter to limit the number of labels returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call `GetCelebrityDetection` and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_celebrity_recognition`][rekognition_get_celebrity_recognition].
#'
#' @usage
...
#' 
#' Amazon Rekognition Video inappropriate or offensive content detection in a stored video is an asynchronous operation. You start analysis by calling [`start_content_moderation`][rekognition_start_content_moderation] which returns a job identifier (`JobId`). When analysis finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_content_moderation`][rekognition_start_content_moderation]. To get the results of the content analysis, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_content_moderation`][rekognition_get_content_moderation] and pass the job identifier (`JobId`) from the initial call to [`start_content_moderation`][rekognition_start_content_moderation].
#' 
#' For more information, see Working with Stored Videos in the Amazon Rekognition Devlopers Guide.
#' 
#' [`get_content_moderation`][rekognition_get_content_moderation] returns detected inappropriate, unwanted, or offensive content moderation labels, and the time they are detected, in an array, `ModerationLabels`, of ContentModerationDetection objects.
#' 
#' By default, the moderated labels are returned sorted by time, in milliseconds from the start of the video. You can also sort them by moderated label by specifying `NAME` for the `SortBy` input parameter.
#' 
#' Since video analysis can return a large number of results, use the `MaxResults` parameter to limit the number of labels returned in a single call to [`get_content_moderation`][rekognition_get_content_moderation]. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call [`get_content_moderation`][rekognition_get_content_moderation] and populate the `NextToken` request parameter with the value of `NextToken` returned from the previous call to [`get_content_moderation`][rekognition_get_content_moderation].
#' 
...
#' 
#' Face detection with Amazon Rekognition Video is an asynchronous operation. You start face detection by calling [`start_face_detection`][rekognition_start_face_detection] which returns a job identifier (`JobId`). When the face detection operation finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_face_detection`][rekognition_start_face_detection]. To get the results of the face detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_face_detection`][rekognition_get_face_detection] and pass the job identifier (`JobId`) from the initial call to [`start_face_detection`][rekognition_start_face_detection].
#' 
#' [`get_face_detection`][rekognition_get_face_detection] returns an array of detected faces (`Faces`) sorted by the time the faces were detected.
#' 
#' Use MaxResults parameter to limit the number of labels returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call [`get_face_detection`][rekognition_get_face_detection] and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_face_detection`][rekognition_get_face_detection].
#' 
#' Note that for the [`get_face_detection`][rekognition_get_face_detection] operation, the returned values for `FaceOccluded` and `EyeDirection` will always be "null".
#'
#' @usage
...
#' 
#' Face search in a video is an asynchronous operation. You start face search by calling to [`start_face_search`][rekognition_start_face_search] which returns a job identifier (`JobId`). When the search operation finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_face_search`][rekognition_start_face_search]. To get the search results, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_face_search`][rekognition_get_face_search] and pass the job identifier (`JobId`) from the initial call to [`start_face_search`][rekognition_start_face_search].
#' 
...
#' 
#' [`get_face_search`][rekognition_get_face_search] only returns the default facial attributes (`BoundingBox`, `Confidence`, `Landmarks`, `Pose`, and `Quality`). The other facial attributes listed in the `Face` object of the following response syntax are not returned. For more information, see FaceDetail in the Amazon Rekognition Developer Guide.
#' 
...
#' 
#' The label detection operation is started by a call to [`start_label_detection`][rekognition_start_label_detection] which returns a job identifier (`JobId`). When the label detection operation finishes, Amazon Rekognition publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to `StartlabelDetection`.
#' 
#' To get the results of the label detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_label_detection`][rekognition_get_label_detection] and pass the job identifier (`JobId`) from the initial call to [`start_label_detection`][rekognition_start_label_detection].
#' 
#' [`get_label_detection`][rekognition_get_label_detection] returns an array of detected labels (`Labels`) sorted by the time the labels were detected. You can also sort by the label name by specifying `NAME` for the `SortBy` input parameter. If there is no `NAME` specified, the default sort is by timestamp.
#' 
...
#' 
#' Use `MaxResults` parameter to limit the number of labels returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call `GetlabelDetection` and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_label_detection`][rekognition_get_label_detection].
#' 
...
#' 
#' The person path tracking operation is started by a call to [`start_person_tracking`][rekognition_start_person_tracking] which returns a job identifier (`JobId`). When the operation finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_person_tracking`][rekognition_start_person_tracking].
#' 
#' To get the results of the person path tracking operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_person_tracking`][rekognition_get_person_tracking] and pass the job identifier (`JobId`) from the initial call to [`start_person_tracking`][rekognition_start_person_tracking].
#' 
#' [`get_person_tracking`][rekognition_get_person_tracking] returns an array, `Persons`, of tracked persons and the time(s) their paths were tracked in the video.
#' 
#' [`get_person_tracking`][rekognition_get_person_tracking] only returns the default facial attributes (`BoundingBox`, `Confidence`, `Landmarks`, `Pose`, and `Quality`). The other facial attributes listed in the `Face` object of the following response syntax are not returned.
#' 
#' For more information, see FaceDetail in the Amazon Rekognition Developer Guide.
#' 
#' By default, the array is sorted by the time(s) a person's path is tracked in the video. You can sort by tracked persons by specifying `INDEX` for the `SortBy` input parameter.
#' 
#' Use the `MaxResults` parameter to limit the number of items returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call [`get_person_tracking`][rekognition_get_person_tracking] and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_person_tracking`][rekognition_get_person_tracking].
#'
#' @usage
...
#' 
#' Segment detection with Amazon Rekognition Video is an asynchronous operation. You start segment detection by calling [`start_segment_detection`][rekognition_start_segment_detection] which returns a job identifier (`JobId`). When the segment detection operation finishes, Amazon Rekognition publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_segment_detection`][rekognition_start_segment_detection]. To get the results of the segment detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. if so, call [`get_segment_detection`][rekognition_get_segment_detection] and pass the job identifier (`JobId`) from the initial call of [`start_segment_detection`][rekognition_start_segment_detection].
#' 
#' [`get_segment_detection`][rekognition_get_segment_detection] returns detected segments in an array (`Segments`) of SegmentDetection objects. `Segments` is sorted by the segment types specified in the `SegmentTypes` input parameter of [`start_segment_detection`][rekognition_start_segment_detection]. Each element of the array includes the detected segment, the precentage confidence in the acuracy of the detected segment, the type of the segment, and the frame in which the segment was detected.
#' 
#' Use `SelectedSegmentTypes` to find out the type of segment detection requested in the call to [`start_segment_detection`][rekognition_start_segment_detection].
#' 
#' Use the `MaxResults` parameter to limit the number of segment detections returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call [`get_segment_detection`][rekognition_get_segment_detection] and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_segment_detection`][rekognition_get_segment_detection].
#' 
...
#' 
#' Text detection with Amazon Rekognition Video is an asynchronous operation. You start text detection by calling [`start_text_detection`][rekognition_start_text_detection] which returns a job identifier (`JobId`) When the text detection operation finishes, Amazon Rekognition publishes a completion status to the Amazon Simple Notification Service topic registered in the initial call to [`start_text_detection`][rekognition_start_text_detection]. To get the results of the text detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. if so, call [`get_text_detection`][rekognition_get_text_detection] and pass the job identifier (`JobId`) from the initial call of [`start_label_detection`][rekognition_start_label_detection].
#' 
#' [`get_text_detection`][rekognition_get_text_detection] returns an array of detected text (`TextDetections`) sorted by the time the text was detected, up to 100 words per frame of video.
#' 
#' Each element of the array includes the detected text, the precentage confidence in the acuracy of the detected text, the time the text was detected, bounding box information for where the text was located, and unique identifiers for words and their lines.
#' 
#' Use MaxResults parameter to limit the number of text detections returned. If there are more results than specified in `MaxResults`, the value of `NextToken` in the operation response contains a pagination token for getting the next set of results. To get the next page of results, call [`get_text_detection`][rekognition_get_text_detection] and populate the `NextToken` request parameter with the token value returned from the previous call to [`get_text_detection`][rekognition_get_text_detection].
#'
#' @usage
...
#' 
#' Amazon Rekognition doesn't save the actual faces that are detected. Instead, the underlying detection algorithm first detects the faces in the input image. For each face, the algorithm extracts facial features into a feature vector, and stores it in the backend database. Amazon Rekognition uses feature vectors when it performs face match and search operations using the [`search_faces`][rekognition_search_faces] and [`search_faces_by_image`][rekognition_search_faces_by_image] operations.
#' 
#' For more information, see Adding faces to a collection in the Amazon Rekognition Developer Guide.
#' 
#' To get the number of faces in a collection, call [`describe_collection`][rekognition_describe_collection].
#' 
#' If you're using version 1.0 of the face detection model, [`index_faces`][rekognition_index_faces] indexes the 15 largest faces in the input image. Later versions of the face detection model index the 100 largest faces in the input image.
#' 
...
#' 
#' To determine which version of the model you're using, call [`describe_collection`][rekognition_describe_collection] and supply the collection ID. You can also get the model version from the value of `FaceModelVersion` in the response from [`index_faces`][rekognition_index_faces]
#' 
#' For more information, see Model Versioning in the Amazon Rekognition Developer Guide.
#' 
#' If you provide the optional `ExternalImageId` for the input image you provided, Amazon Rekognition associates this ID with all faces that it detects. When you call the [`list_faces`][rekognition_list_faces] operation, the response returns the external ID. You can use this external image ID to create a client-side index to associate the faces with each image. You can then use the index to find all faces in an image.
#' 
#' You can specify the maximum number of faces to index with the `MaxFaces` input parameter. This is useful when you want to index the largest faces in an image and don't want to index smaller faces, such as those belonging to people standing in the background.
#' 
#' The `QualityFilter` input parameter allows you to filter out detected faces that don’t meet a required quality bar. The quality bar is based on a variety of common use cases. By default, [`index_faces`][rekognition_index_faces] chooses the quality bar that's used to filter faces. You can also explicitly choose the quality bar. Use `QualityFilter`, to set the quality bar by specifying `LOW`, `MEDIUM`, or `HIGH`. If you do not want to filter detected faces, specify `NONE`.
#' 
#' To use quality filtering, you need a collection associated with version 3 of the face model or higher. To get the version of the face model associated with a collection, call [`describe_collection`][rekognition_describe_collection].
#' 
...
#' 
#' In response, the [`index_faces`][rekognition_index_faces] operation returns an array of metadata for all detected faces, `FaceRecords`. This includes:
#' 
...
#' 
#' If you provide the same image, specify the same collection, and use the same external ID in the [`index_faces`][rekognition_index_faces] operation, Amazon Rekognition doesn't save duplicate face metadata.
#' 
...
#' 
#' To attach a project policy to a project, call [`put_project_policy`][rekognition_put_project_policy]. To remove a project policy from a project, call [`delete_project_policy`][rekognition_delete_project_policy].
#' 
...
#' This operation applies only to Amazon Rekognition Custom Labels.
#' 
#' Attaches a project policy to a Amazon Rekognition Custom Labels project in a trusting AWS account. A project policy specifies that a trusted AWS account can copy a model version from a trusting AWS account to a project in the trusted AWS account. To copy a model version you use the [`copy_project_version`][rekognition_copy_project_version] operation. Only applies to Custom Labels projects.
#' 
...
#' 
#' The response from [`put_project_policy`][rekognition_put_project_policy] is a revision ID for the project policy. You can attach multiple project policies to a project. You can also update an existing project policy by specifying the policy revision ID of the existing policy.
#' 
#' To remove a project policy from a project, call [`delete_project_policy`][rekognition_delete_project_policy]. To get a list of project policies attached to a project, call [`list_project_policies`][rekognition_list_project_policies].
#' 
#' You copy a model version by calling [`copy_project_version`][rekognition_copy_project_version].
#' 
...
#' 
#' You can also search faces without indexing faces by using the [`search_faces_by_image`][rekognition_search_faces_by_image] operation.
#' 
...
#' 
#' To search for all faces in an input image, you might first call the [`index_faces`][rekognition_index_faces] operation, and then use the face IDs returned in subsequent calls to the [`search_faces`][rekognition_search_faces] operation.
#' 
#' You can also call the [`detect_faces`][rekognition_detect_faces] operation and use the bounding boxes in the response to make face crops, which then you can pass in to the [`search_faces_by_image`][rekognition_search_faces_by_image] operation.
#' 
#' You pass the input image either as base64-encoded image bytes or as a reference to an image in an Amazon S3 bucket. If you use the AWS CLI to call Amazon Rekognition operations, passing image bytes is not supported. The image must be either a PNG or JPEG formatted file.
#' 
#' The response returns an array of faces that match, ordered by similarity score with the highest similarity first. More specifically, it is an array of metadata for each face match found. Along with the metadata, the response also includes a `similarity` indicating how similar the face is to the input face. In the response, the operation also returns the bounding box (and a confidence level that the bounding box contains a face) of the face that Amazon Rekognition used for the input image.
#' 
#' If no faces are detected in the input image, [`search_faces_by_image`][rekognition_search_faces_by_image] returns an `InvalidParameterException` error.
#' 
...
#' Starts asynchronous recognition of celebrities in a stored video.
#' 
#' Amazon Rekognition Video can detect celebrities in a video must be stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_celebrity_recognition`][rekognition_start_celebrity_recognition] returns a job identifier (`JobId`) which you use to get the results of the analysis. When celebrity recognition analysis is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`. To get the results of the celebrity recognition analysis, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_celebrity_recognition`][rekognition_get_celebrity_recognition] and pass the job identifier (`JobId`) from the initial call to [`start_celebrity_recognition`][rekognition_start_celebrity_recognition].
#' 
...
#' 
#' Amazon Rekognition Video can moderate content in a video stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_content_moderation`][rekognition_start_content_moderation] returns a job identifier (`JobId`) which you use to get the results of the analysis. When content analysis is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`.
#' 
...
#' Starts asynchronous detection of faces in a stored video.
#' 
#' Amazon Rekognition Video can detect faces in a video stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_face_detection`][rekognition_start_face_detection] returns a job identifier (`JobId`) that you use to get the results of the operation. When face detection is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`. To get the results of the face detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_face_detection`][rekognition_get_face_detection] and pass the job identifier (`JobId`) from the initial call to [`start_face_detection`][rekognition_start_face_detection].
#' 
...
#' 
#' The video must be stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_face_search`][rekognition_start_face_search] returns a job identifier (`JobId`) which you use to get the search results once the search has completed. When searching is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`. To get the search results, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_face_search`][rekognition_get_face_search] and pass the job identifier (`JobId`) from the initial call to [`start_face_search`][rekognition_start_face_search]. For more information, see [Searching stored videos for faces](https://docs.aws.amazon.com/rekognition/latest/dg/procedure-person-search-videos.html).
#'
#' @usage
...
#' 
#' The video must be stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_label_detection`][rekognition_start_label_detection] returns a job identifier (`JobId`) which you use to get the results of the operation. When label detection is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`.
#' 
#' To get the results of the label detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call [`get_label_detection`][rekognition_get_label_detection] and pass the job identifier (`JobId`) from the initial call to [`start_label_detection`][rekognition_start_label_detection].
#' 
#' *Optional Parameters*
#' 
#' [`start_label_detection`][rekognition_start_label_detection] has the `GENERAL_LABELS` Feature applied by default. This feature allows you to provide filtering criteria to the `Settings` parameter. You can filter with sets of individual labels or with label categories. You can specify inclusive filters, exclusive filters, or a combination of inclusive and exclusive filters. For more information on filtering, see [Detecting labels in a video](https://docs.aws.amazon.com/rekognition/latest/dg/labels-detecting-labels-video.html).
#' 
...
#' 
#' Amazon Rekognition Video can track the path of people in a video stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_person_tracking`][rekognition_start_person_tracking] returns a job identifier (`JobId`) which you use to get the results of the operation. When label detection is finished, Amazon Rekognition publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`.
#' 
...
#' 
#' Once the model is running, you can detect custom labels in new images by calling [`detect_custom_labels`][rekognition_detect_custom_labels].
#' 
#' You are charged for the amount of time that the model is running. To stop a running model, call [`stop_project_version`][rekognition_stop_project_version].
#' 
...
#' Starts asynchronous detection of segment detection in a stored video.
#' 
#' Amazon Rekognition Video can detect segments in a video stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_segment_detection`][rekognition_start_segment_detection] returns a job identifier (`JobId`) which you use to get the results of the operation. When segment detection is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`.
#' 
...
#' 
#' To get the results of the segment detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. if so, call [`get_segment_detection`][rekognition_get_segment_detection] and pass the job identifier (`JobId`) from the initial call to [`start_segment_detection`][rekognition_start_segment_detection].
#' 
#' For more information, see Detecting video segments in stored video in the Amazon Rekognition Developer Guide.
#'
#' @usage
...
#' Starts asynchronous detection of text in a stored video.
#' 
#' Amazon Rekognition Video can detect text in a video stored in an Amazon S3 bucket. Use Video to specify the bucket name and the filename of the video. [`start_text_detection`][rekognition_start_text_detection] returns a job identifier (`JobId`) which you use to get the results of the operation. When text detection is finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple Notification Service topic that you specify in `NotificationChannel`.
#' 
#' To get the results of the text detection operation, first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. if so, call [`get_text_detection`][rekognition_get_text_detection] and pass the job identifier (`JobId`) from the initial call to [`start_text_detection`][rekognition_start_text_detection].
#'
#' @usage
...
#' This operation applies only to Amazon Rekognition Custom Labels.
#' 
#' Stops a running model. The operation might take a while to complete. To check the current status, call [`describe_project_versions`][rekognition_describe_project_versions]. Only applies to Custom Labels projects.
#' 
...
#' 
#' [`update_dataset_entries`][rekognition_update_dataset_entries] returns immediatly, but the dataset update might take a while to complete. Use [`describe_dataset`][rekognition_describe_dataset] to check the current status. The dataset updated successfully if the value of `Status` is `UPDATE_COMPLETE`.
#' 
#' To check if any non-terminal errors occured, call [`list_dataset_entries`][rekognition_list_dataset_entries] and check for the presence of `errors` lists in the JSON Lines.
#' 
