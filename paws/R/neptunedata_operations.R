...
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#' @param clean If set to `TRUE`, this flag specifies that all Neptune ML S3 artifacts should be deleted when the job is stopped. The default is `FALSE`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#' @param clean If set to `TRUE`, this flag specifies that all Amazon S3 artifacts should be deleted when the job is stopped. The default is `FALSE`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#' @param clean If this flag is set to `TRUE`, all Neptune ML S3 artifacts should be deleted when the job is stopped. The default is `FALSE`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' @param silent If set to `TRUE`, causes the cancelation of the openCypher query to happen silently.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param id A unique identifier for the new inference endpoint. The default is an autogenerated timestamped name.
#' @param mlModelTrainingJobId The job Id of the completed model-training job that has created the model that the inference endpoint will point to. You must supply either the `mlModelTrainingJobId` or the `mlModelTransformJobId`.
#' @param mlModelTransformJobId The job Id of the completed model-transform job. You must supply either the `mlModelTrainingJobId` or the `mlModelTransformJobId`.
#' @param update If set to `true`, `update` indicates that this is an update request. The default is `false`. You must supply either the `mlModelTrainingJobId` or the `mlModelTransformJobId`.
#' @param neptuneIamRoleArn The ARN of an IAM role providing Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will be thrown.
...
#' @param instanceType The type of Neptune ML instance to use for online servicing. The default is `ml.m5.xlarge`. Choosing the ML instance for an inference endpoint depends on the task type, the graph size, and your budget.
#' @param instanceCount The minimum number of Amazon EC2 instances to deploy to an endpoint for prediction. The default is 1
#' @param volumeEncryptionKMSKey The Amazon Key Management Service (Amazon KMS) key that SageMaker uses to encrypt data on the storage volume attached to the ML compute instances that run the training job. The default is None.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' Neptune fast reset is a two-step process. First you call [`execute_fast_reset`][neptunedata_execute_fast_reset] with `action` set to `initiateDatabaseReset`. This returns a UUID token which you then include when calling [`execute_fast_reset`][neptunedata_execute_fast_reset] again with `action` set to `performDatabaseReset`. See [Empty an Amazon Neptune DB cluster using the fast reset API](https://docs.aws.amazon.com/neptune/latest/userguide/manage-console-fast-reset.html).
#' 
...
#' @param results If this flag is set to `TRUE`, the query results are gathered and displayed as part of the profile report. If `FALSE`, only the result count is displayed.
#' @param chop If non-zero, causes the results string to be truncated at that number of characters. If set to zero, the string contains all the results.
#' @param serializer If non-null, the gathered results are returned in a serialized response message in the format specified by this parameter. See [Gremlin profile API in Neptune](https://docs.aws.amazon.com/neptune/latest/userguide/gremlin-profile-api.html) for more information.
#' @param indexOps If this flag is set to `TRUE`, the results include a detailed report of all index operations that took place during query execution and serialization.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param gremlinQuery &#91;required&#93; Using this API, you can run Gremlin queries in string format much as you can using the HTTP endpoint. The interface is compatible with whatever Gremlin version your DB cluster is using (see the [Tinkerpop client section](https://docs.aws.amazon.com/neptune/latest/userguide/access-graph-gremlin-client.html#best-practices-gremlin-java-latest) to determine which Gremlin releases your engine version supports).
...
#' @param explainMode &#91;required&#93; The openCypher `explain` mode. Can be one of: `static`, `dynamic`, or `details`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' @param parameters The openCypher query parameters for query execution. See [Examples of openCypher parameterized queries](https://docs.aws.amazon.com/neptune/latest/userguide/opencypher-parameterized-queries.html) for more information.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' See [Neptune Loader Get-Status API](https://docs.aws.amazon.com/neptune/latest/userguide/) for more information.
#' 
...
#' @param details Flag indicating whether or not to include details beyond the overall status (`TRUE` or `FALSE`; the default is `FALSE`).
#' @param errors Flag indicating whether or not to include a list of errors encountered (`TRUE` or `FALSE`; the default is `FALSE`).
#' 
#' The list of errors is paged. The `page` and `errorsPerPage` parameters allow you to page through all the errors.
#' @param page The error page number (a positive integer; the default is `1`). Only valid when the `errors` parameter is set to `TRUE`.
#' @param errorsPerPage The number of errors returned in each page (a positive integer; the default is `10`). Only valid when the `errors` parameter set to `TRUE`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param queryId &#91;required&#93; The unique ID of the openCypher query for which to retrieve the query status.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' Gets a stream for a property graph.
#' 
#' With the Neptune Streams feature, you can generate a complete sequence of change-log entries that record every change made to your graph data as it happens. [`get_propertygraph_stream`][neptunedata_get_propertygraph_stream] lets you collect these change-log entries for a property graph.
#' 
...
#' @param commitNum The commit number of the starting record to read from the change-log stream. This parameter is required when `iteratorType` is`AT_SEQUENCE_NUMBER` or `AFTER_SEQUENCE_NUMBER`, and ignored when `iteratorType` is `TRIM_HORIZON` or `LATEST`.
#' @param opNum The operation sequence number within the specified commit to start reading from in the change-log stream data. The default is `1`.
...
#' Gets a stream for an RDF graph.
#' 
#' With the Neptune Streams feature, you can generate a complete sequence of change-log entries that record every change made to your graph data as it happens. [`get_sparql_stream`][neptunedata_get_sparql_stream] lets you collect these change-log entries for an RDF graph.
#' 
#' The Neptune streams feature needs to be enabled on your Neptune DBcluster. To enable streams, set the [neptune_streams](https://docs.aws.amazon.com/neptune/latest/userguide/parameters.html#parameters-db-cluster-parameters-neptune_streams) DB cluster parameter to `1`.
#' 
#' See [Capturing graph changes in real time using Neptune streams](https://docs.aws.amazon.com/neptune/latest/userguide/streams.html).
...
#'
#' @param includeWaiting If set to `TRUE`, the list returned includes waiting queries. The default is `FALSE`;
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param limit The number of load IDs to list. Must be a positive integer greater than zero and not more than `100` (which is the default).
#' @param includeQueuedLoads An optional parameter that can be used to exclude the load IDs of queued load requests when requesting a list of load IDs by setting the parameter to `FALSE`. The default value is `TRUE`.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param maxItems The maximum number of items to return (from 1 to 1024; the default is 10).
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#'
#' @return
#' A list with the following syntax:
...
#'
#' @param includeWaiting When set to `TRUE` and other parameters are not present, causes status information to be returned for waiting queries as well as for running queries.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param mode The statistics generation mode. One of: `DISABLE_AUTOCOMPUTE`, `ENABLE_AUTOCOMPUTE`, or `REFRESH`, the last of which manually triggers DFE statistics generation.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param source &#91;required&#93; The `source` parameter accepts an S3 URI that identifies a single file, multiple files, a folder, or multiple folders. Neptune loads every data file in any folder that is specified.
#' 
...
#' @param format &#91;required&#93; The format of the data. For more information about data formats for the Neptune `Loader` command, see [Load Data Formats](https://docs.aws.amazon.com/neptune/latest/userguide/bulk-load-tutorial-format.html).
#' 
#' **Allowed values**
#' 
...
#' @param edgeOnlyLoad **`edgeOnlyLoad`** - A flag that controls file processing order during bulk loading.
#' 
#' *Allowed values*: `"TRUE"`, `"FALSE"`.
#' 
#' *Default value*: `"FALSE"`.
...
#'
#' @param id A unique identifier for the new job. The default is an autogenerated UUID.
#' @param previousDataProcessingJobId The job ID of a completed data processing job run on an earlier version of the data.
#' @param inputDataS3Location &#91;required&#93; The URI of the Amazon S3 location where you want SageMaker to download the data needed to run the data processing job.
#' @param processedDataS3Location &#91;required&#93; The URI of the Amazon S3 location where you want SageMaker to save the results of a data processing job.
#' @param sagemakerIamRoleArn The ARN of an IAM role for SageMaker execution. This must be listed in your DB cluster parameter group or an error will occur.
#' @param neptuneIamRoleArn The Amazon Resource Name (ARN) of an IAM role that SageMaker can assume to perform tasks on your behalf. This must be listed in your DB cluster parameter group or an error will occur.
#' @param processingInstanceType The type of ML instance used during data processing. Its memory should be large enough to hold the processed dataset. The default is the smallest ml.r5 type whose memory is ten times larger than the size of the exported graph data on disk.
#' @param processingInstanceVolumeSizeInGB The disk volume size of the processing instance. Both input data and processed data are stored on disk, so the volume size must be large enough to hold both data sets. The default is 0. If not specified or 0, Neptune ML chooses the volume size automatically based on the data size.
#' @param processingTimeOutInSeconds Timeout in seconds for the data processing job. The default is 86,400 (1 day).
#' @param modelType One of the two model types that Neptune ML currently supports: heterogeneous graph models (`heterogeneous`), and knowledge graph (`kge`). The default is none. If not specified, Neptune ML chooses the model type automatically based on the data.
#' @param configFileName A data specification file that describes how to load the exported graph data for training. The file is automatically generated by the Neptune export toolkit. The default is `training-data-configuration.json`.
...
#' @param volumeEncryptionKMSKey The Amazon Key Management Service (Amazon KMS) key that SageMaker uses to encrypt data on the storage volume attached to the ML compute instances that run the training job. The default is None.
#' @param s3OutputEncryptionKMSKey The Amazon Key Management Service (Amazon KMS) key that SageMaker uses to encrypt the output of the processing job. The default is none.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param id A unique identifier for the new job. The default is An autogenerated UUID.
#' @param previousModelTrainingJobId The job ID of a completed model-training job that you want to update incrementally based on updated data.
#' @param dataProcessingJobId &#91;required&#93; The job ID of the completed data-processing job that has created the data that the training will work with.
#' @param trainModelS3Location &#91;required&#93; The location in Amazon S3 where the model artifacts are to be stored.
#' @param sagemakerIamRoleArn The ARN of an IAM role for SageMaker execution.This must be listed in your DB cluster parameter group or an error will occur.
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#' @param baseProcessingInstanceType The type of ML instance used in preparing and managing training of ML models. This is a CPU instance chosen based on memory requirements for processing the training data and model.
#' @param trainingInstanceType The type of ML instance used for model training. All Neptune ML models support CPU, GPU, and multiGPU training. The default is `ml.p3.2xlarge`. Choosing the right instance type for training depends on the task type, graph size, and your budget.
...
#' @param maxHPOParallelTrainingJobs Maximum number of parallel training jobs to start for the hyperparameter tuning job. The default is 2. The number of parallel jobs you can run is limited by the available resources on your training instance.
#' @param subnets The IDs of the subnets in the Neptune VPC. The default is None.
#' @param securityGroupIds The VPC security group IDs. The default is None.
#' @param volumeEncryptionKMSKey The Amazon Key Management Service (KMS) key that SageMaker uses to encrypt data on the storage volume attached to the ML compute instances that run the training job. The default is None.
#' @param s3OutputEncryptionKMSKey The Amazon Key Management Service (KMS) key that SageMaker uses to encrypt the output of the processing job. The default is none.
#' @param enableManagedSpotTraining Optimizes the cost of training machine-learning models by using Amazon Elastic Compute Cloud spot instances. The default is `False`.
...
#'
#' @param id A unique identifier for the new job. The default is an autogenerated UUID.
#' @param dataProcessingJobId The job ID of a completed data-processing job. You must include either `dataProcessingJobId` and a `mlModelTrainingJobId`, or a `trainingJobName`.
#' @param mlModelTrainingJobId The job ID of a completed model-training job. You must include either `dataProcessingJobId` and a `mlModelTrainingJobId`, or a `trainingJobName`.
#' @param trainingJobName The name of a completed SageMaker training job. You must include either `dataProcessingJobId` and a `mlModelTrainingJobId`, or a `trainingJobName`.
#' @param modelTransformOutputS3Location &#91;required&#93; The location in Amazon S3 where the model artifacts are to be stored.
#' @param sagemakerIamRoleArn The ARN of an IAM role for SageMaker execution. This must be listed in your DB cluster parameter group or an error will occur.
#' @param neptuneIamRoleArn The ARN of an IAM role that provides Neptune access to SageMaker and Amazon S3 resources. This must be listed in your DB cluster parameter group or an error will occur.
#' @param customModelTransformParameters Configuration information for a model transform using a custom model. The `customModelTransformParameters` object contains the following fields, which must have values compatible with the saved model parameters from the training job:
#' @param baseProcessingInstanceType The type of ML instance used in preparing and managing training of ML models. This is an ML compute instance chosen based on memory requirements for processing the training data and model.
