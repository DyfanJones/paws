...
#' 
#' There are two advantages to using [`add_endpoints`][globalaccelerator_add_endpoints] to add endpoints in Global Accelerator:
#' 
...
#' 
#' To stop advertising the BYOIP address range, use [WithdrawByoipCidr](https://docs.aws.amazon.com/global-accelerator/latest/api/).
#' 
...
#' 
#' Before you can release an address range, you must stop advertising it by using [`withdraw_byoip_cidr`][globalaccelerator_withdraw_byoip_cidr] and you must not have any accelerators that are using static IP addresses allocated from its address range.
#' 
#' For more information, see [Bring your own IP addresses (BYOIP)](https://docs.aws.amazon.com/global-accelerator/latest/dg/using-byoip.html) in the *Global Accelerator Developer Guide*.
#'
#' @usage
...
#' Remove endpoints from an endpoint group.
#' 
#' The [`remove_endpoints`][globalaccelerator_remove_endpoints] API operation is the recommended option for removing endpoints. The alternative is to remove endpoints by updating an endpoint group by using the [`update_endpoint_group`][globalaccelerator_update_endpoint_group] API operation. There are two advantages to using [`add_endpoints`][globalaccelerator_add_endpoints] to remove endpoints instead:
#' 
#' -   It's more convenient, because you only need to specify the endpoints that you want to remove. With the [`update_endpoint_group`][globalaccelerator_update_endpoint_group] API operation, you must specify all of the endpoints in the endpoint group except the ones that you want to remove from the group.
#' 
#' -   It's faster, because Global Accelerator doesn't need to resolve any endpoints. With the [`update_endpoint_group`][globalaccelerator_update_endpoint_group] API operation, Global Accelerator must resolve all of the endpoints that remain in the group.
#'
#' @usage
