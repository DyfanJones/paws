...
#' This operation aborts a multipart upload identified by the upload ID.
#' 
#' After the Abort Multipart Upload request succeeds, you cannot upload any more parts to the multipart upload or complete the multipart upload. Aborting a completed upload fails. However, aborting an already-aborted upload will succeed, for a short time. For more information about uploading a part and completing a multipart upload, see [`upload_multipart_part`][glacier_upload_multipart_part] and [`complete_multipart_upload`][glacier_complete_multipart_upload].
#' 
#' This operation is idempotent.
#' 
#' An AWS account has full permission to perform all operations (actions). However, AWS Identity and Access Management (IAM) users don't have any permissions by default. You must grant them explicit permission to perform specific actions. For more information, see [Access Control Using AWS Identity and Access Management (IAM)](https://docs.aws.amazon.com/amazonglacier/latest/dev/security-iam.html).
#' 
...
#' 
#' A vault lock is put into the `InProgress` state by calling [`initiate_vault_lock`][glacier_initiate_vault_lock]. A vault lock is put into the `Locked` state by calling [`complete_vault_lock`][glacier_complete_vault_lock]. You can get the state of a vault lock by calling [`get_vault_lock`][glacier_get_vault_lock]. For more information about the vault locking process, see [Amazon Glacier Vault Lock](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock.html). For more information about vault lock policies, see [Amazon Glacier Access Control with Vault Lock Policies](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock-policy.html).
#' 
...
#' @param accountId &#91;required&#93; The `AccountId` value is the AWS account ID. This value must match the AWS account ID associated with the credentials used to sign the request. You can either specify an AWS account ID or optionally a single '`-`' (hyphen), in which case Amazon Glacier uses the AWS account ID associated with the credentials used to sign the request. If you specify your account ID, do not include any hyphens ('-') in the ID.
#' @param vaultName &#91;required&#93; The name of the vault.
#' @param lockId &#91;required&#93; The `lockId` value is the lock ID obtained from a [`initiate_vault_lock`][glacier_initiate_vault_lock] request.
#'
#' @return
#' An empty list.
#'
...
#' 
#' A vault lock is put into the `InProgress` state by calling [`initiate_vault_lock`][glacier_initiate_vault_lock]. A vault lock is put into the `Locked` state by calling [`complete_vault_lock`][glacier_complete_vault_lock]. You can abort the vault locking process by calling [`abort_vault_lock`][glacier_abort_vault_lock]. For more information about the vault locking process, [Amazon Glacier Vault Lock](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock.html).
#' 
...
#' 
#' For information about setting a notification configuration on a vault, see [`set_vault_notifications`][glacier_set_vault_notifications]. If a notification configuration for a vault is not set, the operation returns a `404 Not Found` error. For more information about vault notifications, see [Configuring Vault Notifications in Amazon Glacier](https://docs.aws.amazon.com/amazonglacier/latest/dev/configuring-notifications.html).
#' 
#' An AWS account has full permission to perform all operations (actions). However, AWS Identity and Access Management (IAM) users don't have any permissions by default. You must grant them explicit permission to perform specific actions. For more information, see [Access Control Using AWS Identity and Access Management (IAM)](https://docs.aws.amazon.com/amazonglacier/latest/dev/security-iam.html).
#' 
...
#' 
#' Every part you upload to this resource (see [`upload_multipart_part`][glacier_upload_multipart_part]), except the last one, must have the same size. The last one can be the same size or smaller. For example, suppose you want to upload a 16.2 MB file. If you initiate the multipart upload with a part size of 4 MB, you will upload four parts of 4 MB each and one part of 0.2 MB.
#' 
...
#' 
#' You must complete the vault locking process within 24 hours after the vault lock enters the `InProgress` state. After the 24 hour window ends, the lock ID expires, the vault automatically exits the `InProgress` state, and the vault lock policy is removed from the vault. You call [`complete_vault_lock`][glacier_complete_vault_lock] to complete the vault locking process by setting the state of the vault lock to `Locked`.
#' 
#' After a vault lock is in the `Locked` state, you cannot initiate a new vault lock for the vault.
#' 
#' You can abort the vault locking process by calling [`abort_vault_lock`][glacier_abort_vault_lock]. You can get the state of the vault lock by calling [`get_vault_lock`][glacier_get_vault_lock]. For more information about the vault locking process, [Amazon Glacier Vault Lock](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock.html).
#' 
#' If this operation is called when the vault lock is in the `InProgress` state, the operation returns an `AccessDeniedException` error. When the vault lock is in the `InProgress` state you must call [`abort_vault_lock`][glacier_abort_vault_lock] before you can initiate a new vault lock policy.
#'
#' @usage
...
#' 
#' Note the difference between this operation and listing parts ([`list_parts`][glacier_list_parts]). The List Multipart Uploads operation lists all multipart uploads for a vault and does not require a multipart upload ID. The List Parts operation requires a multipart upload ID since parts are associated with a single upload.
#' 
#' An AWS account has full permission to perform all operations (actions). However, AWS Identity and Access Management (IAM) users don't have any permissions by default. You must grant them explicit permission to perform specific actions. For more information, see [Access Control Using AWS Identity and Access Management (IAM)](https://docs.aws.amazon.com/amazonglacier/latest/dev/security-iam.html).
#' 
...
#' 
#' -   **ArchiveRetrievalCompleted** This event occurs when a job that was initiated for an archive retrieval is completed ([`initiate_job`][glacier_initiate_job]). The status of the completed job can be "Succeeded" or "Failed". The notification sent to the SNS topic is the same output as returned from [`describe_job`][glacier_describe_job].
#' 
#' -   **InventoryRetrievalCompleted** This event occurs when a job that was initiated for an inventory retrieval is completed ([`initiate_job`][glacier_initiate_job]). The status of the completed job can be "Succeeded" or "Failed". The notification sent to the SNS topic is the same output as returned from [`describe_job`][glacier_describe_job].
#' 
#' An AWS account has full permission to perform all operations (actions). However, AWS Identity and Access Management (IAM) users don't have any permissions by default. You must grant them explicit permission to perform specific actions. For more information, see [Access Control Using AWS Identity and Access Management (IAM)](https://docs.aws.amazon.com/amazonglacier/latest/dev/security-iam.html).
#' 
...
#' 
#' You must use the archive ID to access your data in Amazon Glacier. After you upload an archive, you should save the archive ID returned so that you can retrieve or delete the archive later. Besides saving the archive ID, you can also index it and give it a friendly name to allow for better searching. You can also use the optional archive description field to specify how the archive is referred to in an external index of archives, such as you might create in Amazon DynamoDB. You can also get the vault inventory to obtain a list of archive IDs in a vault. For more information, see [`initiate_job`][glacier_initiate_job].
#' 
...
#' 
#' You can optionally specify an archive description of up to 1,024 printable ASCII characters. You can get the archive description when you either retrieve the archive or get the vault inventory. For more information, see [`initiate_job`][glacier_initiate_job]. Amazon Glacier does not interpret the description in any way. An archive description does not need to be unique. You cannot use the description to retrieve or sort the archive list.
#' 
...
#' 
#' -   **Part size does not match**The size of each part except the last must match the size specified in the corresponding [`initiate_multipart_upload`][glacier_initiate_multipart_upload] request. The size of the last part must be the same size as, or smaller than, the specified size.
#' 
